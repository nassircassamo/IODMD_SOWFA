function [u,predictedpower,Pref]=power_referencetracking_quadprog(mpcmodel,Hp,Hc,Inputs,Outputs,scalingfactors,dirdmd,qq,rr)
tic
...
%% Generate reference for tracking
 ...
 Pref_wt(:,2) = [0*ones(100,1); 0.25*ones(200,1); 0.5*ones(200,1); 1*ones(200,1); 2*ones(200+Hp+1,1)];
...
for k=1:length(Pref)-Hp
    ...
    u_opt=quadprog(HH,cc,Arest,brest,[],[],[],[],[]);
