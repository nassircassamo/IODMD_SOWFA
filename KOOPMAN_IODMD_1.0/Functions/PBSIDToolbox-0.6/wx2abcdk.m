function [A,B,C,D,K] = wx2abcdk(x,u,y,f,p,c)
%WX2ABCDK Estimates the matrices A, B, C and D of the state space model
%  [A,B,C,D] = wx2abcdk(x,u,g1y,f,p) estimates the matrices A, B, C and D
%  of the state space model:
%
%       x(k+1) = A x(k) + B u(k) + K e(k)
%       g(y(k))^-1 = C x(k) + D u(k) + e(k)
%
...
%
%  [A,B,C,D] = wx2abcdk(x,u,g1y,f,p,'stable') estimates a stable matrix A
%  by using the method in [1].
%
%  [A,B,C,D,K] = wx2abcdk(x,u,g1y,f,p) returns a Kalman matrix K
%  calculated by the discrete Riccati equation, which gives a guaranteed
%  stable A-KC (predictor from).
%
%  See also: wmodx.m, wordvarx.m, wx2abck.m.
%
%  References:
...
if nargin < 5
    error('WX2ABCDK requires at least five input arguments.')
end
...
for k = 1:batch
    ...
    n = size(x,1);
    if l == 0
        error('WX2ABCDK requires an output vector y.')
    end
    if n == 0
        error('WX2ABCDK requires an state vector x.')
    end
