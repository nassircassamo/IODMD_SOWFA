...
%  [A,B,C,D] = dx2abcdk(x,u,y,f,p) estimates the matrices A, B, C and D of
...
%
%  [A,B,C,D] = dx2abcdk(x,u,y,f,p,'stable1') forces to estimate a stable 
%  matrix A by using the method in [1].
%
%  [A,B,C,D] = dx2abcdk(x,u,y,f,p,'stable2') forces to estimate a stable 
%  matrix A by using the method in [2].
%
%  [A,B,C,D,K] = dx2abcdk(x,u,y,f,p) returns a Kalman matrix K calculated
...
for k = 1:batch
    ...
    if nargout > 4 && ~strcmpi(c,'nostable')
