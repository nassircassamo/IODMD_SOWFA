function [A,B,C,K] = px2abck(x,u,y,mu,f,p,pind,c)
%PX2ABCK Estimates the matrices A, B, and C of the state space model
%  [A,B,C,K]=dx2abc(x,u,y,f,p,c,pind) estimates the matrices A, B, and C of
%  the state space model:
%
%       x(k+1) = A kron(mu(k),x(k)) + B kron(mu(k),u(k)) + K(j) e(k)
%       y(k)   = C kron(mu(k),x(k)) + e(k)
%
...
%  parameter-varying matrices A, B, and C, where matrices have the form
%  of A=[A(1) A(2) ... A(m)].
%
%  [A,B,C,K]=dx2abc(x,u,y,f,p,c,pind,c) specifies which of the system
%  matrices are constant and not parameter-varing. For each of the matrices
%  A, B, and C an 1 or 0 can be given in the vector c. If the A matrix is
%  assumed constant the C matrix cannot be assumed constant and also visa
%  versa. (default C=[0 0 0])
%
%  References:
...
%
%  See also: pvordarx, pmodx.m, px2abcdk.m.
...
if nargin < 7
    error('PX2ABCK requires at least seven input arguments.')
end
if nargin < 8
    c = zeros(1,3);
end
