...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_sim1_rlti_varmax_gust2_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_sim1_rlti_varmax_gust2_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_sim1_rlti_varmax_gust2_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "sim1_rlti_varmax_gust2") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_sim1_rlti_varmax_gust2_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int sf_sim1_rlti_varmax_gust2_get_eml_resolved_functions_info(
  ...
  if (strcmp(machineName, "sim1_rlti_varmax_gust2") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_sim1_rlti_varmax_gust2_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_sim1_rlti_varmax_gust2_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"sim1_rlti_varmax_gust2")) {
    return(sf_sim1_rlti_varmax_gust2_method_dispatcher(simstructPtr,
...
extern void sim1_rlti_varmax_gust2_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    sim1_rlti_varmax_gust2_terminator();
    sf_debug_terminate();
  }
...
extern void sim1_rlti_varmax_gust2_initializer(void);
extern void sim1_rlti_varmax_gust2_register_exported_symbols(SimStruct* S);
extern void sim1_rlti_varmax_gust2_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  sim1_rlti_varmax_gust2_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      sim1_rlti_varmax_gust2_debug_initialize();
    }

    sim1_rlti_varmax_gust2_initializer();
  }
