function x = wmodx(X,n)
%WMODX  Closed-loop Wiener system identification using the PBSIDopt method.
%  x=wmodx(X,n) estimates the state sequence x of the identifiable system
%  with order n. The order n can be determined from the singular values
%  given by wordvarx. The matrix X is also calculated by wordvarx.
%
%  See also: wordvarx, wx2abcdk.m, and wx2abck.m.
%
%  References:
