function [A,B,C,D,K] = hwx2abcdk(x,u,y,f,p,c)
%HWX2ABCDK Estimates the matrices A, B, C and D of the state space model
%  [A,B,C,D] = hwx2abcdk(x,fu,g1y,f,p) estimates the matrices A, B, C and D
%  of the state space model:
%
%       x(k+1) = A x(k) + B f(u(k)) + K e(k)
%       g(y(k))^-1   = C x(k) + D f(u(k)) + e(k)
%
...
%
%  [A,B,C,D] = hwx2abcdk(x,fu,g1y,f,p,'stable') estimates a stable matrix A
%  by using the method in [1].
%
%  [A,B,C,D,K] = hwx2abcdk(x,fu,g1y,f,p) returns a Kalman matrix K
%  calculated by the discrete Riccati equation, which gives a guaranteed
%  stable A-KC (predictor from).
%
%  See also: hwmodx.m, hwordvarx.m, hwx2abck.m.
%
%  References:
...
for k = 1:batch
    ...
    n = size(x,1);
    if l == 0
        error('DX2ABCDK requires an output vector y.')
    end
    if n == 0
        error('DX2ABCDK requires an state vector x.')
    end
