function sys = d2c(sys,method,approx,varargin)
%IDAFFLPV/D2C  Discrete-to-continuous conversion of state-space models.
%  MC = D2C(MD,METHOD,APPROX) converts the discrete-time IDAFFLPV
%  model MC to a continuous-time model MD with sample time Ts. The string
...
%  MC = D2C(MD,'prewarp',APPROX,Wc). The string APPROX selects the
...
switch lower(method)
    case 'zoh'
        ...
        ae = logm(as)./Ts;
        be = ae*inv(as - eye(nx))*bs;
        ke = ae*inv(as - eye(nx))*ks;
    ...
    case {'tustin','prewarp'}
        ...
        at = (2/T).*(eye(nx) - as)/(eye(nx) + as);
        bt = (1/sqrt(T)).*((eye(nx) - (eye(nx) - as)/(eye(nx) + as))*bs);
        kt = (1/sqrt(T)).*((eye(nx) - (eye(nx) - as)/(eye(nx) + as))*ks);
        ct = (1/sqrt(T)).*(cs*(eye(nx) - (eye(nx) - as)/(eye(nx) + as)));
        dt = ds - (1/2).*(cs*(eye(nx) - (eye(nx) - as)/(eye(nx) + as))*bs);
