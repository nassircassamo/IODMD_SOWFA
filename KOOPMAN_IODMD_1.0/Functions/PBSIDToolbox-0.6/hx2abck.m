function [A,B,C,K] = hx2abck(x,u,y,f,p,c)
%HX2ABCK Estimates the matrices A, B, and C of the state space model
%  [A,B,C] = hx2abck(x,fu,y,f,p) estimates the matrices A, B, and C of the
%  state space model:
%
...
%
%  [A,B,C] = hx2abck(x,fu,y,f,p,'stable') estimates a stable matrix A by
%  using the method in [1].
%
%  [A,B,C,K] = hx2abck(x,fu,y,f,p) returns a Kalman matrix K calculated by
%  the discrete Riccati equation, which gives a guaranteed stable A-KC
%  (predictor from).
%
%  See also: hmodx.m, hordvarx.m, hx2abcdk.m.
%
%  References:
