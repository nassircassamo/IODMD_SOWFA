function [B,D] = fac2bd(A,C,varargin) 
%FAC2BD     Frequency domain estimation of B and D based on A, C 
%           and measured FRF. 
...
if nargin < 4
    error('FAC2BD requires at least four input arguments.')
end
if rem(nargin-2,2) ~= 0
...
for Batch = 1:Nbatch
    ...
    Phi = zeros(N(Batch)*l,n+l);
    Phi(:,1:n) = ltifrf((A/wscale)',(C/wscale)',eye(n),[],[],w/wscale,2).';

    for i = 1:l
        Phi(i:l:(N(Batch)-1)*l+i,n+i) = ones(N(Batch),1);
    end
