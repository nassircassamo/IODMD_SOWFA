/* IB01PD.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ib01pd(char *meth, char *job, char *jobcv, integer *
...
/*     Decode the scalar input parameters. */
    ...
    n4sid = lsame(meth, "N");
    withal = lsame(job, "A");
    withc = lsame(job, "C") || withal;
    withd = lsame(job, "D") || withal;
    withb = lsame(job, "B") || withd;
...
/*         following subroutine, as returned by ILAENV.) */
	...
	iaw = 0;
	minwrk = ldunn + (*n << 2);
	maxwrk = ldunn + *n + *n * ilaenv(&c__1, "DGEQRF", " ", &ldun2, n, &
...
/* Computing MAX */
		i__1 = maxwrk, i__2 = (ldunn << 1) + *n + *n * ilaenv(&c__1, 
			"DORMQR", "LT", &ldun2, n, n, &c_n1, (ftnlen)6, (ftnlen)2);
...
/* Computing MAX */
	    ...
	    i__1 = minwrk, i__2 = ldunn + iaw + n2 + max(i__3,lmmnol), i__1 = 
		    max(i__1,i__2), i__2 = id + (mnobrn << 2), i__1 = max(
		    i__1,i__2), i__2 = id + mnobrn + npl;
	    minwrk = max(i__1,i__2);
/* Computing MAX */
/* Computing MAX */
	    i__3 = *n * ilaenv(&c__1, "DGEQRF", " ", &lnobr, n, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1), i__4 = lmmnob * ilaenv(&c__1, 
		    "DORMQR", "LT", &lnobr, &lmmnob, n, &c_n1, (ftnlen)6, (ftnlen)2), i__3 = max(i__3,i__4), i__4 = lmmnol * ilaenv(
		    &c__1, "DORMQR", "LT", &ldun2, &lmmnol, n, &c_n1, (ftnlen)6, (ftnlen)2);
	    i__1 = maxwrk, i__2 = ldunn + iaw + n2 + max(i__3,i__4), i__1 = 
		    max(i__1,i__2), i__2 = id + *n + *n * ilaenv(&c__1, 
		    "DGEQRF", " ", &lmnobr, n, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1), i__1 = max(i__1,i__2), i__2 = id + *n + npl * 
		    ilaenv(&c__1, "DORMQR", "LT", &lmnobr, &npl, n, &c_n1, (ftnlen)6, (ftnlen)2);
	    maxwrk = max(i__1,i__2);
	    if (n4sid && (*m > 0 && withb)) {
/* Computing MAX */
/* Computing MAX */
...
/*     Copy  un1 = GaL = Un(1:(s-1)*L,1:n)  in the workspace. */
    ...
    igal = 1;
    dlacpy("Full", &ldun2, n, &r__[nr2 + nr2 * r_dim1], ldr, &dwork[igal], &
	    ldun2);
...
/*                prefer L*(NOBR-1)*N+N+N*NB. */
    ...
    ldw = jwork;
    i__1 = *ldwork - jwork + 1;
    dgeqrf(&ldun2, n, &dwork[igal], &ldun2, &dwork[itau1], &dwork[jwork], &
	    i__1, &ierr);

/*     Compute the reciprocal of the condition number of r1. */
/*     Workspace: need L*(NOBR-1)*N+4*N. */

    dtrcon("1-norm", "Upper", "NonUnit", n, &dwork[igal], &ldun2, &rcond1, &
...
/*           Workspace: need   2*L*(NOBR-1)*N+N. */
	    ...
	    dlacpy("Full", l, n, &r__[nr2 + nr2 * r_dim1], ldr, &c__[
		    c_offset], ldc);
	    dlacpy("Full", &ldun2, n, &r__[nr2 + *l + nr2 * r_dim1], ldr, &
		    dwork[iun2], &ldun2);
...
/*                      prefer 2*L*(NOBR-1)*N+N+N*NB. */
	    ...
	    jwork = iun2 + ldunn;
	    i__1 = *ldwork - jwork + 1;
	    dormqr("Left", "Transpose", &ldun2, n, n, &dwork[igal], &ldun2, &
		    dwork[itau1], &dwork[iun2], &ldun2, &dwork[jwork], &i__1, 
		    &ierr);
	    dlacpy("Full", n, n, &dwork[iun2], &ldun2, &a[a_offset], lda);
	    ncol = *n;
	    jwork = iun2;
	}
...
/*           Solve for  A  (if requested),  r1*A = un2(1:n,:)  in  A. */
	    ...
	    if (withc) {
		dtrtrs("Upper", "NoTranspose", "NonUnit", n, n, &dwork[igal],
			 &ldun2, &a[a_offset], lda, &ierr);
		if (ierr > 0) {
		    *info = 3;
		    return 0;
		}
...
/*              Workspace: need   2*L*(NOBR-1)*N+2*N+N*N. */
		...
		ihous = jwork;
		jwork = ihous + ldunn;
		dlacpy("Lower", &ldun2, n, &dwork[igal], &ldun2, &dwork[
			ihous], &ldun2);
	    } else {
		ihous = igal;
	    }
...
/*                         prefer 2*L*(NOBR-1)*N+N*N+2*N+N*NB. */
		...
		i__1 = ldun2 - *n;
		dlaset("Full", n, &i__1, &c_b66, &c_b66, &r__[nr3 + (nr2 + *
			n) * r_dim1], ldr);
		i__1 = *ldwork - jwork + 1;
		dormqr("Right", "Transpose", n, &ldun2, n, &dwork[ihous], &
...
/*                 Save  pinv(GaL)  in  DWORK(IGAL). */
		    ...
		    dlacpy("Full", n, &ldun2, &r__[nr3 + nr2 * r_dim1], ldr, 
			    &dwork[igal], n);
		}
		jwork = iun2;
	    }
...
/*           Workspace: need L*(NOBR-1)*N+N+3*M*NOBR. */
	    ...
	    dtrcon("1-norm", "Upper", "NonUnit", &mnobr, &r__[nr3 + r_dim1], 
		    ldr, &rcond2, &dwork[jwork], &iwork[1], &ierr);
...
/*           where  u2 = Un(:,n+1:L*s),  and  K  is  (Ls-n) x ms. */
	    ...
	    dgemm("Transpose", "Transpose", &lnobrn, &mnobr, &lnobr, &c_b63, 
		    &r__[nr2 + (nr2 + *n) * r_dim1], ldr, &r__[nr2 * r_dim1 + 
		    1], ldr, &c_b66, &r__[nr2 + nr3 * r_dim1], ldr);
...
/*              rank. Solve for  K,  K*R_1c' = u2'*R_2c'. */
		...
		dtrsm("Right", "Upper", "Transpose", "Non-unit", &lnobrn, &
...
/*                      prefer larger. */
	    ...
	    if (withco) {
		dlacpy("Full", &lnobr, n, &r__[nr2 + nr2 * r_dim1], ldr, &o[
			o_offset], ldo);
	    }
	    i__1 = *ldwork - jwork + 1;
	    ib01py(meth, jobpy, nobr, n, m, l, &rank, &r__[nr2 + nr2 * 
...
/* Computing MAX */
	    ...
	    if (withco) {
		dlacpy("Full", &lnobr, n, &o[o_offset], ldo, &r__[nr2 + 
			r_dim1], ldr);
	    }
...
/*     matrix of left singular vectors,  Un,  in  R_21  and in  O. */
	...
	if (*m > 0) {
	    dlacpy("Full", &lnobr, n, &r__[nr2 + nr2 * r_dim1], ldr, &r__[
		    nr2 + r_dim1], ldr);
	}
	dlacpy("Full", &lnobr, n, &r__[nr2 + nr2 * r_dim1], ldr, &o[o_offset]
		, ldo);
    }
...
/*                Aw = 0,      otherwise. */
    ...
    jwork = itau2 + *n;
    i__1 = *ldwork - jwork + 1;
    dgeqrf(&lnobr, n, &r__[nr2 + r_dim1], ldr, &dwork[itau2], &dwork[jwork], 
	    &i__1, &ierr);
...
/*                prefer L*(NOBR-1)*N+Aw+2*N+(2*M+L)*NOBR*NB. */
	...
	ma02ad("Full", &lmmnob, &lnobr, &r__[nr4 * r_dim1 + 1], ldr, &r__[
		nr4 + r_dim1], ldr);
    }
    i__1 = *ldwork - jwork + 1;
    dormqr("Left", "Transpose", &lnobr, &lmmnob, n, &r__[nr2 + r_dim1], ldr, 
...
/*     solution  Y  in the second block-column of  R. */
    ...
    dtrtrs("Upper", "NoTranspose", "NonUnit", n, &lmmnob, &r__[nr2 + r_dim1],
	     ldr, &r__[nr4 + r_dim1], ldr, &ierr);
    if (ierr > 0) {
	*info = 3;
	return 0;
    }
    ma02ad("Full", n, &lmmnob, &r__[nr4 + r_dim1], ldr, &r__[nr2 * r_dim1 + 
	    1], ldr);
    nr4mn = nr4 - *n;
    nr4pl = nr4 + *l;
    nrow = lmmnol;
...
/*                   prefer L*(NOBR-1)*N+Aw+2*N+((2*M+L)*NOBR + L)*NB. */
	...
	ma02ad("Full", &lmmnol, &ldun2, &r__[nr4pl * r_dim1 + 1], ldr, &r__[
		nr4pl + r_dim1], ldr);
	i__1 = *ldwork - jwork + 1;
	dormqr("Left", "Transpose", &ldun2, &lmmnol, n, &dwork[igal], &ldun2,
...
/*        into the last part of the third block-column of  R. */
	...
	dtrtrs("Upper", "NoTranspose", "NonUnit", n, &lmmnol, &dwork[igal], &
		ldun2, &r__[nr4pl + r_dim1], ldr, &ierr);
	if (ierr > 0) {
	    *info = 3;
	    return 0;
	}
	...
	if (shift) {
		...
		dcopy(&lmmnol, &r__[(nr4 + i__) * r_dim1 + 1], &c__1, &r__[(
			nr4 + *n + i__) * r_dim1 + 1], &c__1);
/* L10: */
	    }

	}
	ma02ad("Full", n, &lmmnol, &r__[nr4pl + r_dim1], ldr, &r__[nr4mn * 
		r_dim1 + 1], ldr);
	nrow = 0;
    }
...
/*                             prefer larger. */
	...
	if (moesp) {
	    *(unsigned char *)fact = 'F';
	    if (*m > 0 && withb) {
		dlacpy("Full", n, &ldun2, &dwork[igal], n, &r__[nr4pl + nr2 *
			 r_dim1], ldr);
	    }
	} else {
...
/*           Save the elementary reflectors used for computing r1. */
	    ...
	    ihous = jwork;
	    dlacpy("Lower", &ldun2, n, &dwork[igal], &ldun2, &dwork[ihous], &
		    ldun2);
	...
	i__1 = *ldwork - jwork + 1;
	mb02ud(fact, "Right", "Transpose", jobp, &nrow, n, &c_b63, &toll1, &
	...
	if (nrow > 0) {
	    if (shift) {
		nr4mn = nr4;
		dlacpy("Full", &lmmnol, l, &r__[nr4 * r_dim1 + 1], ldr, &r__[
			(nr4 - *l) * r_dim1 + 1], ldr);
		dlacpy("Full", &lmmnol, n, &r__[nr4pl * r_dim1 + 1], ldr, &
			r__[nr4mn * r_dim1 + 1], ldr);
		dlacpy("Full", &lmmnol, l, &r__[(nr4 - *l) * r_dim1 + 1], 
			ldr, &r__[(nr4 + *n) * r_dim1 + 1], ldr);
	    } else {
		dlacpy("Full", &lmmnol, n, &r__[nr4pl * r_dim1 + 1], ldr, &
			r__[nr4mn * r_dim1 + 1], ldr);
	    }
...
/*                      prefer 2*L*(NOBR-1)*N+2*N+N*NB. */
	    ...
	    jwork = iu;
	    i__1 = ldun2 - *n;
	    dlaset("Full", n, &i__1, &c_b66, &c_b66, &r__[nr4pl + (nr2 + *n) 
		    * r_dim1], ldr);
	    i__1 = *ldwork - jwork + 1;
	    dormqr("Right", "Transpose", n, &ldun2, n, &dwork[ihous], &ldun2,
...
/*     where  d = 0,  for  MOESP,  and  d = 1,  for  N4SID. */
	...
	dcopy(n, &dwork[itau2], &c__1, &dwork[1], &c__1);
	itau = *n + 1;
    }

    jwork = itau + *n;
    i__1 = *ldwork - jwork + 1;
    dgeqrf(&lmnobr, n, &r__[nr2 + nr2 * r_dim1], ldr, &dwork[itau], &dwork[
	    jwork], &i__1, &ierr);
...
/*                prefer d*N+N+(N+L)*NB. */
    ...
    i__1 = *ldwork - jwork + 1;
    dormqr("Left", "Transpose", &lmnobr, &npl, n, &r__[nr2 + nr2 * r_dim1], 
	    ldr, &dwork[itau], &r__[nr2 + nr4mn * r_dim1], ldr, &dwork[jwork],
	     &i__1, &ierr);

    i__1 = *l - 1;
    i__2 = *l - 1;
    dlaset("Lower", &i__1, &i__2, &c_b66, &c_b66, &r__[nr4 + 1 + nr4 * 
	    r_dim1], ldr);
...
/*     Workspace: need d*N+3*(M*NOBR+N). */
    ...
    jwork = itau;
    dtrcon("1-norm", "Upper", "NonUnit", &mnobrn, &r__[r_offset], ldr, &
	    rcond3, &dwork[jwork], &iwork[1], info);
...
/*        Solve for  V(m*s+1:m*s+n,:),  giving  [ A'  C' ]. */
	...
	rankm = mnobrn;
	if (n4sid) {
	    dtrsm("Left", "Upper", "NoTranspose", "NonUnit", n, &npl, &c_b63,
		     &r__[nr2 + nr2 * r_dim1], ldr, &r__[nr2 + nr4mn * r_dim1], ldr);
	}
    } else {
	fullr = FALSE_;
...
/* L20: */
	...
	if (n4sid && (*m > 0 && withb)) {
	    dlacpy("Full", &lnobr, n, &r__[nr2 + r_dim1], ldr, &r__[nr4 + 
		    r_dim1], ldr);
	}
	jwork = itau + mnobrn;
	i__1 = mnobrn - 1;
	dlaset("Lower", &i__1, &mnobrn, &c_b66, &c_b66, &r__[r_dim1 + 2], 
		ldr);
	mb03od("QR", &mnobrn, &mnobrn, &r__[r_offset], ldr, &iwork[1], &toll,
		 &svlmax, &dwork[itau], &rankm, sval, &dwork[jwork], &ierr);

/*        Workspace: need   d*N+M*NOBR+N+N+L; */
/*                   prefer d*N+M*NOBR+N+(N+L)*NB. */

	i__1 = *ldwork - jwork + 1;
	dormqr("Left", "Transpose", &mnobrn, &npl, &mnobrn, &r__[r_offset], 
...
/*        RANKM = m*s+n.) */
	...
	if (moesp) {
	    i__1 = lmmnol - rankm;
	    dsyrk("Upper", "Transpose", &npl, &i__1, &rnrm, &r__[rankm + 1 + 
		    nr4mn * r_dim1], ldr, &c_b66, &r__[r_offset], ldr);
	    dlacpy("Upper", n, n, &r__[r_offset], ldr, &q[q_offset], ldq);
	    dlacpy("Full", n, l, &r__[(*n + 1) * r_dim1 + 1], ldr, &s[
		    s_offset], lds);
	    dlacpy("Upper", l, l, &r__[*n + 1 + (*n + 1) * r_dim1], ldr, &ry[
		    ry_offset], ldry);
	} else {
	    i__1 = lmmnol - rankm;
	    dsyrk("Upper", "Transpose", &npl, &i__1, &rnrm, &r__[rankm + 1 + 
		    nr4mn * r_dim1], ldr, &c_b66, &dwork[jwork], &npl);
	    dlacpy("Upper", n, n, &dwork[jwork], &npl, &q[q_offset], ldq);
	    dlacpy("Full", n, l, &dwork[jwork + *n * npl], &npl, &s[s_offset]
		    , lds);
	    dlacpy("Upper", l, l, &dwork[jwork + *n * (npl + 1)], &npl, &ry[
		    ry_offset], ldry);
	}
	ma02ed("Upper", n, &q[q_offset], ldq);
	ma02ed("Upper", l, &ry[ry_offset], ldry);

/*        Check the magnitude of the residual. */

	i__1 = lmmnol - rankm;
	rnrm = dlange("1-norm", &i__1, &npl, &r__[rankm + 1 + nr4mn * r_dim1]
		, ldr, &dwork[jwork]);
	if (rnrm < thresh) {
	    *iwarn = 5;
	}
...
/*                      prefer larger. */
	    ...
	    mb03od("No QR", n, n, &r__[nr2 + nr2 * r_dim1], ldr, &iwork[1], &
		    toll1, &svlmax, &dwork[itau], &rankm, sval, &dwork[jwork],
		     &ierr);
	    i__1 = *ldwork - jwork + 1;
	    mb02qy(n, n, &npl, &rankm, &r__[nr2 + nr2 * r_dim1], ldr, &iwork[
...
/* Computing MAX */
	    ...
	    jwork = itau;
	    if (*m > 0 && withb) {
		dlacpy("Full", &lnobr, n, &r__[nr4 + r_dim1], ldr, &r__[nr2 
			+ r_dim1], ldr);
	    }
	}
...
/*           implicitly used. */
	    ...
	    ma02ad("Full", n, n, &r__[nr2 + nr4mn * r_dim1], ldr, &a[
		    a_offset], lda);
	    ma02ad("Full", n, l, &r__[nr2 + (nr4mn + *n) * r_dim1], ldr, &
		    c__[c_offset], ldc);
	} else {

/*           Use the given  A  and  C. */

	    ma02ad("Full", n, n, &a[a_offset], lda, &r__[nr2 + nr4mn * 
		    r_dim1], ldr);
	    ma02ad("Full", l, n, &c__[c_offset], ldc, &r__[nr2 + (nr4mn + *n)
		     * r_dim1], ldr);
	}
...
/*           m*s  rows of  R(1,NR4MN). */
	    ...
	    dgemm("NoTranspose", "NoTranspose", &mnobr, &npl, n, &c_b173, &
		    r__[nr2 * r_dim1 + 1], ldr, &r__[nr2 + nr4mn * r_dim1], 
		    ldr, &c_b63, &r__[nr4mn * r_dim1 + 1], ldr);
...
/*           Then, solve the least squares problem. */
	    ...
	    dlacpy("Full", n, n, &a[a_offset], lda, &r__[nr2 + nr4 * r_dim1],
		     ldr);
	    dlacpy("Full", l, n, &c__[c_offset], ldc, &r__[nr2 + *n + nr4 * 
		    r_dim1], ldr);
	    dtrsm("Right", "Upper", "NoTranspose", "NonUnit", &npl, n, &
		    c_b63, &r__[nr2 + r_dim1], ldr, &r__[nr2 + nr4 * r_dim1], 
		    ldr);
	    dlaset("Full", &npl, &lnobrn, &c_b66, &c_b66, &r__[nr2 + (nr4 + *
		    n) * r_dim1], ldr);

/*           Workspace: need 2*N+L; prefer N + (N+L)*NB. */

	    i__1 = *ldwork - jwork + 1;
	    dormqr("Right", "Transpose", &npl, &lnobr, n, &r__[nr2 + r_dim1],
...
/*                      prefer larger. */
	    ...
	    ma02ad("Full", &mnobr, &npl, &r__[nr4mn * r_dim1 + 1], ldr, &r__[
		    nr2 + nr3 * r_dim1], ldr);
/* Computing 2nd power */
	    i__1 = npl;
	    ...
	    i__1 = mnobr * npl;
	    i__2 = *ldwork - jwork + 1;
	    ib01px(jobpy, nobr, n, m, l, &r__[r_offset], ldr, &o[o_offset], 
