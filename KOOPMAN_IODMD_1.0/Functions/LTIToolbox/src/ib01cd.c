/* IB01CD.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ib01cd(char *jobx0, char *comuse, char *job, integer *n,
...
/*     Check the input parameters. */
	...
	*info = -1;
    } else if (! (compbd || usebd || lsame(comuse, "N")
	    )) {
	*info = -2;
    } else if (! withb) {
	*info = -3;
...
/* Computing MAX */
		    i__5 = *nsmp - *m;
		    i__3 = *m * ilaenv(&c__1, "DGEQRF", " ", nsmp, m, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1), i__4 = ncol + ncol *
			     ilaenv(&c__1, "DGEQRF", " ", &i__5, &ncol, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1);
		    i__1 = maxwrk, i__2 = isize + *n + *m + max(i__3,i__4);
		    maxwrk = max(i__1,i__2);
/* Computing MAX */
/* Computing MAX */
		    i__5 = *nsmp - *m;
		    i__3 = ncp1 * ilaenv(&c__1, "DORMQR", "LT", nsmp, &ncp1, m, &c_n1, (ftnlen)6, (ftnlen)2), i__4 = ncol + 
			    ilaenv(&c__1, "DORMQR", "LT", &i__5, &c__1, &ncol, &c_n1, (ftnlen)6, (ftnlen)2);
		    i__1 = maxwrk, i__2 = isize + *n + *m + max(i__3,i__4);
		    maxwrk = max(i__1,i__2);
		} else {
/* Computing MAX */
/* Computing MAX */
		    i__3 = ncol * ilaenv(&c__1, "DGEQRF", " ", &nsmpl, &ncol, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1), i__4 = 
			    ilaenv(&c__1, "DORMQR", "LT", &nsmpl, &c__1, &ncol, &c_n1, (ftnlen)6, (ftnlen)2);
...
/* Computing MAX */
/* Computing MAX */
		i__3 = *n * ilaenv(&c__1, "DGEQRF", " ", &nsmpl, n, &c_n1, &
			c_n1, (ftnlen)6, (ftnlen)1), i__4 = ilaenv(&c__1, 
			"DORMQR", "LT", &nsmpl, &c__1, n, &c_n1, (ftnlen)6, (ftnlen)2);
...
/*                 prefer larger. */
    ...
    ic = ia + nn;
    ib = ic + ln;
    dlacpy("Full", n, n, &a[a_offset], lda, &dwork[ia], &ldw);
    dlacpy("Full", l, n, &c__[c_offset], ldc, &dwork[ic], l);

    if (usebd) {
	mtmp = *m;
	dlacpy("Full", n, m, &b[b_offset], ldb, &dwork[ib], &ldw);
    } else {
	mtmp = 0;
    }
    iwr = ib + nm;
    ...
    i__1 = *ldwork - jwork + 1;
    tb01wd(n, &mtmp, l, &dwork[ia], &ldw, &dwork[ib], &ldw, &dwork[ic], l, &
...
/* Computing MAX */
	...
	if (dlapy2(&dwork[i__], &dwork[i__ + *n]) >= 1.) {
	    *iwarn = 6;
	}
/* L10: */
    }
...
/*                prefer larger. */
    ...
    if (compbd) {
	i__1 = *ldwork - jwork + 1;
	ib01qd(jobx0, job, n, m, l, nsmp, &dwork[ia], &ldw, &dwork[ic], l, &
...
/*           original system. */
	    ...
	    dgemm("NoTranspose", "NoTranspose", n, m, n, &c_b57, &v[v_offset]
	...
	i__1 = *ldwork - jwork + 1;
	ib01rd(jobd, n, &mtmp, l, nsmp, &dwork[ia], &ldw, &dwork[ib], &ldw, &
...
/*           Workspace: need g + N*N + L*N + N*M + N. */
	    ...
	    dgemv("NoTranspose", n, n, &c_b57, &v[v_offset], ldv, &x0[1], &
		    c__1, &c_b58, &dwork[jwork], &c__1);
	    dcopy(n, &dwork[jwork], &c__1, &x0[1], &c__1);
	}
