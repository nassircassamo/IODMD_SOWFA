/* TB05AD.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int tb05ad(char *baleig, char *inita, integer *n, integer *
...
/*     .. Executable Statements .. */
    ...
    *info = 0;
    ...
    lbaleb = lsame(baleig, "B") || lsame(baleig, "E");
    lbalea = lsame(baleig, "A");
    lbalba = lbaleb || lbalea;
    linita = lsame(inita, "G");

/*     Test the input scalar arguments. */
    if (! lbalec && ! lbalba && ! lsame(baleig, "N")) {
	*info = -1;
    } else if (! linita && ! lsame(inita, "H")) {
	*info = -2;
    } else if (*n < 0) {
	*info = -3;
    } else if (*m < 0) {
...
/*     Quick return if possible. */
    ...
    if (*n == 0) {
	if (min(*m,*p) > 0) {
	    zlaset("Full", p, m, &c_b1, &c_b1, &g[g_offset], ldg);
	}
	*rcond = 1.;
	dwork[1] = 1.;
	return 0;
...
/*        Workspace: need N. */
	...
	dgebal(balanc, n, &a[a_offset], lda, &low, &igh, &dwork[1], info);
	if (lbalba) {
...
/*                    Permute rows of B. */
			...
			if (*m > 0) {
			    dswap(m, &b[jj + b_dim1], ldb, &b[jp + b_dim1], 
				    ldb);
			}
...
/*                    Permute columns of C. */
			...
			if (*p > 0) {
			    dswap(p, &c__[jj * c_dim1 + 1], &c__1, &c__[jp * 
				    c_dim1 + 1], &c__1);
			}
		    }
		}
...
/*                 Scale rows of permuted B. */
		    ...
		    if (*m > 0) {
			d__1 = 1. / t;
			dscal(m, &d__1, &b[j + b_dim1], ldb);
		    }

/*                 Scale columns of permuted C. */

		    if (*p > 0) {
			dscal(p, &t, &c__[j * c_dim1 + 1], &c__1);
		    }
/* L20: */
		}
...
/*        Workspace: need 2*N - 1;  prefer N - 1 + N*NB. */
	...
	jwork = itau + *n - 1;
	i__1 = *ldwork - jwork + 1;
	dgehrd(n, &low, &igh, &a[a_offset], lda, &dwork[itau], &dwork[jwork],
		 &i__1, info);
/* Computing MAX */
...
/*        Workspace: need N - 1 + M;  prefer N - 1 + M*NB. */
	...
	i__1 = *ldwork - jwork + 1;
	dormhr("Left", "Transpose", n, m, &low, &igh, &a[a_offset], lda, &
...
/*        Workspace: need N - 1 + P;  prefer N - 1 + P*NB. */
	...
	i__1 = *ldwork - jwork + 1;
	dormhr("Right", "No transpose", p, n, &low, &igh, &a[a_offset], lda, 
...
/*           Workspace: need N. */
	    ...
	    dhseqr("Eigenvalues", "No Schur", n, &low, &igh, &a[a_offset], 
...
/*        estimation. */
	...
	for (j = 1; j <= i__1; ++j) {
	    i__2 = j - 1;
	    t = z_abs(&zwork[jj]) + dasum(&i__2, &a[j * a_dim1 + 1], &c__1);
	    if (j < *n) {
...
/*     Factor the complex Hessenberg matrix. */
    ...
    mb02sz(n, &zwork[1], n, &iwork[1], info);
    if (*info != 0) {
	*info = 2;
    }
...
/*        Workspace: need 2*N. */
	...
	mb02tz("1-norm", n, &hnorm, &zwork[1], n, &iwork[1], rcond, &dwork[1]
		, &zwork[*n * *n + 1], info);
/* Computing MAX */
...
/* L110: */
    ...
    mb02rz("No transpose", n, m, &zwork[1], n, &iwork[1], &hinvb[
	    hinvb_offset], ldhinv, info);
