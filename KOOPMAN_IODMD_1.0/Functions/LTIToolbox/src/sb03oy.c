/* SB03OY.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int sb03oy(logical *discr, logical *ltrans, integer *isgn, 
...
/* Computing MAX */
    ...
    *scale = 1.;
    ...
    dlanv2(&s11, &s12, &s21, &s22, &tempr, &tempi, &e1, &e2, csp, csq);
    if (tempi == 0.) {
	*info = 4;
	return 0;
    }
...
/*     Compute the cos and sine that define  Qhat.  The sine is real. */
    ...
    sb03ov(temp, &s[s_dim1 + 2], csq, &snq);
...
/*        X22 should be taken. */
	...
	if (tempi == 0.) {
	    ...
	    r__[r_dim1 + 1] = dlapy2(&tempr, &tempi);
...
/*        Obtain u11 from the QR-factorization of X. */
	...
	if (tempi == 0.) {
	    ...
	    r__[(r_dim1 << 1) + 2] = dlapy2(&tempr, &tempi);
