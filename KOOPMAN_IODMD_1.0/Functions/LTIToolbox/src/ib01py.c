/* IB01PY.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ib01py(char *meth, char *job, integer *nobr, integer *n,
...
/* Computing MAX */
	...
	maxwrk = minwrk;
/* Computing MAX */
	i__1 = maxwrk, i__2 = *l + *l * ilaenv(&c__1, "DGEQRF", " ", &nrow, 
		l, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1);
	maxwrk = max(i__1,i__2);
/* Computing MAX */
	i__1 = maxwrk, i__2 = *l + ldun2 * ilaenv(&c__1, "DORMQR", "LT", &
		nrow, &ldun2, l, &c_n1, (ftnlen)6, (ftnlen)2);
	maxwrk = max(i__1,i__2);
/* Computing MAX */
	i__1 = maxwrk, i__2 = *l + mnobr * ilaenv(&c__1, "DORMQR", "LT", &
		nrow, &mnobr, l, &c_n1, (ftnlen)6, (ftnlen)2);
	maxwrk = max(i__1,i__2);

	if (*m > 0 && withb) {
/* Computing MAX */
	    i__1 = minwrk, i__2 = lnobr << 2, i__1 = max(i__1,i__2), i__2 = 
		    lnobr + *m;
	    minwrk = max(i__1,i__2);
/* Computing MAX */
	    i__1 = max(minwrk,maxwrk), i__2 = lnobr + *m * ilaenv(&c__1, 
		    "DORMQR", "LT", &lnobr, m, &lnobr, &c_n1, (ftnlen)6, (ftnlen)2);
	    maxwrk = max(i__1,i__2);
	}
...
/*     Q_1i,  defined above, is  (p/s)-by-L,  for  i = 1:s. */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ma02ad("Full", l, &nrow, &ul[*l * (i__ - 1) + 1 + (*n + 1) * 
...
/*     Workspace: need 2*L, prefer L+L*NB. */
    ...
    i__2 = *ldwork - jwork + 1;
    dgeqrf(&nrow, l, &r__[r_offset], ldr, &dwork[itau], &dwork[jwork], &i__2,
	     &ierr);
...
/*     Workspace: need L*NOBR, prefer L+(L*NOBR-L)*NB. */
    ...
    i__2 = *ldwork - jwork + 1;
    dormqr("Left", "Transpose", &nrow, &ldun2, l, &r__[r_offset], ldr, &
...
/*     Copy  H_1  in  H(1:L,1:m). */
    ...
    dlacpy("Full", l, m, &k[k_offset], ldk, &h__[h_offset], ldh);
...
/*        R  and  F. */
	...
	i__1 = lnobr - *l * i__;
	dlacpy("Upper", l, &i__1, &r__[*l * (i__ - 1) + 1 + (*l * (i__ - 1) 
		+ 1) * r_dim1], ldr, &r__[*l * i__ + 1 + (*l * i__ + 1) * 
		r_dim1], ldr);
	i__1 = lnobr - *l * (i__ + 1);
	mb04od("Full", l, &i__1, &nrowml, &r__[*l * i__ + 1 + (*l * i__ + 1) 
...
/*        the matrix  G  and copy  H(i+1)  in  H(L*i+1:L*(i+1),1:m). */

	i__1 = *l;
	for (j = 1; j <= i__1; ++j) {
	    i__3 = *m * (*nobr - i__);
	    mb04oy(&nrowml, &i__3, &ul[(*l * (i__ - 1) + j) * ul_dim1 + 1], &
		    dwork[j], &k[j + (*m * i__ + 1) * k_dim1], ldk, &k[lp1 + 
		    k_dim1], ldk, &dwork[jwork]);
/* L80: */
	}

	dlacpy("Full", l, m, &k[(*m * i__ + 1) * k_dim1 + 1], ldk, &h__[*l * 
		i__ + 1 + h_dim1], ldh);
/* L90: */
    }
...
/*     Workspace: need 3*L*NOBR. */
    ...
    dtrcon("1-norm", "Upper", "NonUnit", &lnobr, &r__[r_offset], ldr, &rcond,
...
/*        Solve for  X,  R*X = H. */
	...
	dtrsm("Left", "Upper", "NoTranspose", "Non-unit", &lnobr, m, &c_b53, 
		&r__[r_offset], ldr, &h__[h_offset], ldh);
    } else {
...
/*        Workspace: need 4*L*NOBR. */

	jwork = itau + lnobr;
	i__2 = lnobr - 1;
	dlaset("Lower", &i__2, &lnobr, &c_b56, &c_b56, &r__[r_dim1 + 2], ldr);
	mb03od("QR", &lnobr, &lnobr, &r__[r_offset], ldr, &iwork[1], &toll, &
		svlmax, &dwork[itau], &rank, sval, &dwork[jwork], &ierr);

/*        Workspace: need L*NOBR+M; prefer L*NOBR+M*NB. */

	i__2 = *ldwork - jwork + 1;
	dormqr("Left", "Transpose", &lnobr, m, &lnobr, &r__[r_offset], ldr, &
...
/*        Workspace: need L*NOBR+max(L*NOBR,M); prefer larger. */
	...
	i__2 = *ldwork - jwork + 1;
	mb02qy(&lnobr, &lnobr, m, &rank, &r__[r_offset], ldr, &iwork[1], &
...
/*     Construct the matrix  D,  if needed. */
    ...
    if (withd) {
	dlacpy("Full", l, m, &h__[ldun2 + 1 + h_dim1], ldh, &d__[d_offset], 
		ldd);
    }
...
/*     in  H(1:L*s-L,1:n). */
	...
	i__1 = nobrh;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    dswap(l, &h__[*l * (i__ - 1) + 1 + j * h_dim1], &c__1, &h__[*l * 
...
/*        Workspace: need M; prefer M*NB. */
	...
	dormqr("Left", "Transpose", &ldun2, m, n, &r1[r1_offset], ldr1, &
...
/*        Compute the solution in  B. */
	...
	dlacpy("Full", n, m, &h__[h_offset], ldh, &b[b_offset], ldb);

	dtrtrs("Upper", "NoTranspose", "NonUnit", n, m, &r1[r1_offset], ldr1,
		 &b[b_offset], ldb, &ierr);
	if (ierr > 0) {
	    *info = 3;
	    return 0;
...
/*        pseudoinverse of  GaL  for computing  B  from  GaL*B = H. */
	...
	dgemm("NoTranspose", "NoTranspose", n, m, &ldun2, &c_b53, &pgal[
