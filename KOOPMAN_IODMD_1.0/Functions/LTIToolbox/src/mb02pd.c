/* MB02PD.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int mb02pd(char *fact, char *trans, integer *n, integer *
...
/*     Test the input parameters. */
	...
	*info = -8;
    } else if (lsame(fact, "F") && ! (rowequ || colequ 
	    || lsame(equed, "N"))) {
	*info = -10;
    } else {
	if (rowequ) {
...
/*        Compute row and column scalings to equilibrate the matrix A. */
	...
	dgeequ(n, n, &a[a_offset], lda, &r__[1], &c__[1], &rowcnd, &colcnd, &
		amax, &infequ);
	if (infequ == 0) {

/*           Equilibrate the matrix. */

	    dlaqge(n, n, &a[a_offset], lda, &r__[1], &c__[1], &rowcnd, &
		    colcnd, &amax, equed);
	    rowequ = lsame(equed, "R") || lsame(equed,
		     "B");
	    colequ = lsame(equed, "C") || lsame(equed,
		     "B");
	}
    }
...
/*        Compute the LU factorization of A. */
	...
	dlacpy("Full", n, n, &a[a_offset], lda, &af[af_offset], ldaf);
	dgetrf(n, n, &af[af_offset], ldaf, &ipiv[1], info);
...
/*              leading rank-deficient INFO columns of A. */
		...
		rpvgrw = dlantr("M", "U", "N", info, info, &af[af_offset], 
			ldaf, &dwork[1]);
		if (rpvgrw == 0.) {
		    rpvgrw = 1.;
		} else {
		    rpvgrw = dlange("M", n, info, &a[a_offset], lda, &dwork[
			    1]) / rpvgrw;
		}
		dwork[1] = rpvgrw;
		*rcond = 0.;
...
/*        reciprocal pivot growth factor RPVGRW. */
	...
	anorm = dlange(norm, n, n, &a[a_offset], lda, &dwork[1]);
	rpvgrw = dlantr("M", "U", "N", n, n, &af[af_offset], ldaf, &dwork[1]);
	if (rpvgrw == 0.) {
	    rpvgrw = 1.;
	} else {
	    rpvgrw = dlange("M", n, n, &a[a_offset], lda, &dwork[1]) / rpvgrw;
	}

/*        Compute the reciprocal of the condition number of A. */

	dgecon(norm, n, &af[af_offset], ldaf, &anorm, rcond, &dwork[1], &
		iwork[1], info);
...
/*     Compute the solution matrix X. */
    ...
    dlacpy("Full", n, nrhs, &b[b_offset], ldb, &x[x_offset], ldx);
    dgetrs(trans, n, nrhs, &af[af_offset], ldaf, &ipiv[1], &x[x_offset], ldx,
	     info);

/*     Use iterative refinement to improve the computed solution and */
/*     compute error bounds and backward error estimates for it. */

    dgerfs(trans, n, nrhs, &a[a_offset], lda, &af[af_offset], ldaf, &ipiv[1],
