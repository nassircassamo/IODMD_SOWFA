/* MB01RY.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int mb01ry(char *side, char *uplo, char *trans, integer *m, 
...
/*                 column of B. */
		    ...
		    dcopy(&j, &b[j * b_dim1 + 1], &c__1, &dwork[1], &c__1);
		    dtrmv("Upper", trans, "Non-unit", &j, &h__[h_offset], 
			    ldh, &dwork[1], &c__1);
...
/*                    j-th column of B. */
			...
			i__2 = *m - j;
			dgemv(trans, &j, &i__2, beta, &h__[(j + 1) * h_dim1 
				+ 1], ldh, &b[j + 1 + j * b_dim1], &c__1, 
				alpha, &r__[j * r_dim1 + 1], &c__1)
				;
		    } else {
			dscal(m, alpha, &r__[*m * r_dim1 + 1], &c__1);
		    }
...
/*                 of the j-th column of B. */
		    ...
		    i__1 = *m - j + 1;
		    dcopy(&i__1, &b[j + j * b_dim1], &c__1, &dwork[j], &c__1)
			    ;
		    i__1 = *m - j + 1;
		    dtrmv("Upper", trans, "Non-unit", &i__1, &h__[j + j * 
			    h_dim1], ldh, &dwork[j], &c__1);
		    if (j > 1) {
...
/*                    part of the j-th column of B. */
			...
			i__1 = j - 1;
			i__2 = *m - j + 1;
			dgemv(trans, &i__1, &i__2, beta, &h__[j * h_dim1 + 1]
				, ldh, &b[j * b_dim1 + 1], &c__1, alpha, &r__[
				j + j * r_dim1], &c__1);
		    } else {
			dscal(m, alpha, &r__[r_dim1 + 1], &c__1);
		    }
...
/*        TRANS = 'T'. */
	...
	if (luplo) {
	    if (ltrans) {
		r__[r_dim1 + 1] = *alpha * r__[r_dim1 + 1] + *beta * ddot(m, 
			&b[b_offset], ldb, &h__[h_offset], ldh);

		i__1 = *m;
		for (j = 2; j <= i__1; ++j) {
		    i__2 = *m - j + 2;
		    dgemv("NoTranspose", &j, &i__2, beta, &b[(j - 1) * 
...
/* L90: */
		...
		for (j = 1; j <= i__1; ++j) {
		    i__2 = j + 1;
		    dgemv("NoTranspose", &j, &i__2, beta, &b[b_offset], ldb, 
			    &h__[j * h_dim1 + 1], &c__1, alpha, &r__[j * 
			    r_dim1 + 1], &c__1);
/* L100: */
		}

		dgemv("NoTranspose", m, m, beta, &b[b_offset], ldb, &h__[*m *
			 h_dim1 + 1], &c__1, alpha, &r__[*m * r_dim1 + 1], &c__1);
		...
		dgemv("NoTranspose", m, m, beta, &b[b_offset], ldb, &h__[
		...
		for (j = 2; j <= i__1; ++j) {
		    i__2 = *m - j + 1;
		    i__3 = *m - j + 2;
		    dgemv("NoTranspose", &i__2, &i__3, beta, &b[j + (j - 1) *
			     b_dim1], ldb, &h__[j + (j - 1) * h_dim1], ldh, 
			    alpha, &r__[j + j * r_dim1], &c__1);
/* L110: */
		}
		...
		for (j = 1; j <= i__1; ++j) {
		    i__2 = *m - j + 1;
		    i__3 = j + 1;
		    dgemv("NoTranspose", &i__2, &i__3, beta, &b[j + b_dim1], 
...
/* L120: */
		...
		r__[*m + *m * r_dim1] = *alpha * r__[*m + *m * r_dim1] + *
			beta * ddot(m, &b[*m + b_dim1], ldb, &h__[*m * 
			h_dim1 + 1], &c__1);
