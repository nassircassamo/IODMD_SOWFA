/* MB01VD.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int mb01vd(char *trana, char *tranb, integer *ma, integer *
...
/*     Test the input scalar arguments. */
    ...
    /* Function Body */
    transa = lsame(trana, "T") || lsame(trana, "C");
    transb = lsame(tranb, "T") || lsame(tranb, "C");
    *mc = *ma * *mb;
    *info = 0;
    if (! (transa || lsame(trana, "N"))) {
	*info = -1;
    } else if (! (transb || lsame(tranb, "N"))) {
	*info = -2;
    } else if (*ma < 0) {
	*info = -3;
    } else if (*na < 0) {
...
/*     Quick return, if possible. */
    ...
    *nc = *na * *nb;
    ...
    if (*alpha == 0.) {
	if (*beta == 0.) {
	    dlaset("Full", mc, nc, &c_b10, &c_b10, &c__[c_offset], ldc);
	} else if (*beta != 1.) {

	    i__1 = *nc;
	    for (j = 1; j <= i__1; ++j) {
		dscal(mc, beta, &c__[j * c_dim1 + 1], &c__1);
/* L10: */
	    }
...
/*                 Case beta = 0, alpha = 1, A sparse. */
			...
			for (k = 1; k <= i__2; ++k) {
				...
				aij = a[i__ + j * a_dim1];
				if (aij == 0.) {
				    dcopy(mb, dum, &c__0, &c__[ic + jc * 
					    c_dim1], &c__1);
				} else if (aij == 1.) {
				    dcopy(mb, &b[k * b_dim1 + 1], &c__1, &
...
/*                 Case beta <> 0 or 1, alpha = 1, A sparse. */
			...
			for (k = 1; k <= i__2; ++k) {
				...
				if (aij == 0.) {
				    dscal(mb, beta, &c__[ic + jc * c_dim1], &
					    c__1);
				} else {
				    lc = ic;
...
/*                 Case beta = 0, alpha = 1, A sparse. */
			...
			for (k = 1; k <= i__2; ++k) {
				...
				if (aij == 0.) {
				    ...
				    dcopy(mb, &b[k + b_dim1], ldb, &c__[ic + 
					    jc * c_dim1], &c__1);
				} else {
				    lc = ic;
