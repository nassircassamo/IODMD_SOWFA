/* IB01MD.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ib01md(char *meth, char *alg, char *batch, char *conct, 
...
/*     Decode the scalar input parameters. */
    ...
    first = lsame(batch, "F") || onebch;
    interm = lsame(batch, "I");
    last = lsame(batch, "L") || onebch;
    if (! onebch) {
...
/*           following subroutine, as returned by ILAENV.) */
		...
		minwrk = nr << 1;
		maxwrk = nr + nr * ilaenv(&c__1, "DGEQRF", " ", &ns, &nr, &
...
/* L10: */
	    ...
	    if (*m > 0) {
		dlacpy("Full", &nobr21, m, &u[u_offset], ldu, &dwork[nobr2], 
			&ldrwrk);
	    }
	    dlacpy("Full", &nobr21, l, &y[y_offset], ldy, &dwork[ldrwrk * *m 
		    + nobr2], &ldrwrk);
	}

	if (*m > 0) {

/*           Let  Guu(i,j) = Guu0(i,j) + u_i*u_j' + u(i+1)*u(j+1)' + */
/*                                 ... + u(i+NS-1)*u(j+NS-1)', */
...
/*              in backward order. */
		...
		for (i__ = nobr21 * *m; i__ >= 1; --i__) {
		    daxpy(&i__, &c_b24, &r__[i__ * r_dim1 + 1], &c__1, &r__[*
...
/*           Compute/update  Guu(1,1). */
	    ...
	    if (! first && connec) {
		dsyrk("Upper", "Transpose", m, &nobr21, &c_b29, &dwork[1], &
			ldrwrk, &upd, &r__[r_offset], ldr);
	    }
	    dsyrk("Upper", "Transpose", m, &ns, &c_b29, &u[u_offset], ldu, &
		    upd, &r__[r_offset], ldr);
...
/*                 Compute/update  Guu(1,j). */
		    ...
		    dgemm("Transpose", "NoTranspose", m, m, &ns, &c_b29, &u[
...
/*                 block-Hankel structure. */
			...
			for (i__ = jd - *m; i__ <= i__2; ++i__) {
			    dcopy(&i__, &r__[i__ * r_dim1 + 1], &c__1, &r__[*
				    m + 1 + (*m + i__) * r_dim1], &c__1);
/* L30: */
			}
			...
			i__2 = jd - 1;
			for (i__ = jd - *m; i__ <= i__2; ++i__) {
			    daxpy(&i__, &c_b29, &r__[i__ * r_dim1 + 1], &
...
/* L40: */
			...
			dger(m, m, &c_b29, &u[ns + i__ - 1 + u_dim1], ldu, &
				u[ns + j - 1 + u_dim1], ldu, &r__[id + jd * 
				r_dim1], ldr);
			dger(m, m, &c_b24, &u[i__ - 1 + u_dim1], ldu, &u[j - 
...
/* L50: */
			...
			daxpy(&i__, &u[ns + j - 1 + i__ * u_dim1], &u[ns + j 
				- 1 + u_dim1], ldu, &r__[jd + (jd + i__ - 1) *
				 r_dim1], &c__1);
			d__1 = -u[j - 1 + i__ * u_dim1];
			daxpy(&i__, &d__1, &u[j - 1 + u_dim1], ldu, &r__[jd 
...
/*                 with connected blocks. */
		    ...
		    dgemm("Transpose", "NoTranspose", m, m, &nobr21, &c_b29, 
...
/* L90: */
			...
			dger(m, m, &c_b29, &u[ns + i__ - 1 + u_dim1], ldu, &
				u[ns + j - 1 + u_dim1], ldu, &r__[id + jd * 
				r_dim1], ldr);
			dger(m, m, &c_b24, &dwork[i__ - 1], &ldrwrk, &dwork[
...
/* L100: */
			...
			daxpy(&i__, &d__1, &dwork[j - 1], &ldrwrk, &r__[jd + 
				(jd + i__ - 1) * r_dim1], &c__1);
/* L110: */
		    }
...
/*              the (1,2) part is transposed in-situ.) */
		...
		i__1 = mnobr;
		for (j = 2; j <= i__1; ++j) {
		    dswap(&j, &r__[j * r_dim1 + 1], &c__1, &r__[mnobr + 1 + (
			    mnobr + j) * r_dim1], &c__1);
		    i__2 = j - 1;
		    dswap(&i__2, &r__[(mnobr + j) * r_dim1 + 1], &c__1, &r__[
...
/*           Let  Guy(i,j) = Guy0(i,j) + u_i*y_j' + u(i+1)*y(j+1)' + */
/*                                 ... + u(i+NS-1)*y(j+NS-1)', */
...
/*              in backward order. */
		...
		for (i__ = nr - *l; i__ >= i__1; --i__) {
		    daxpy(&ii, &c_b24, &r__[i__ * r_dim1 + 1], &c__1, &r__[*
...
/*           Compute/update the first block-column of  Guy,  Guy(i,1). */
		...
		for (i__ = 1; i__ <= i__1; ++i__) {
		    dgemm("Transpose", "NoTranspose", m, l, &ns, &c_b29, &u[
...
/*                 block-Hankel structure. */
			...
			for (i__ = jd - *l; i__ <= i__2; ++i__) {
			    dcopy(&ii, &r__[i__ * r_dim1 + 1], &c__1, &r__[*
				    m + 1 + (*l + i__) * r_dim1], &c__1);
/* L170: */
			}
			...
			i__2 = jd - 1;
			for (i__ = jd - *l; i__ <= i__2; ++i__) {
			    daxpy(&ii, &c_b29, &r__[i__ * r_dim1 + 1], &c__1,
...
/* L180: */
		    ...
		    i__2 = nobr2;
		    for (i__ = 2; i__ <= i__2; ++i__) {
			dger(m, l, &c_b29, &u[ns + i__ - 1 + u_dim1], ldu, &
				y[ns + j - 1 + y_dim1], ldy, &r__[id + jd * 
				r_dim1], ldr);
			dger(m, l, &c_b24, &u[i__ - 1 + u_dim1], ldu, &y[j - 
...
/*                 with connected blocks. */
		    ...
		    dgemm("Transpose", "NoTranspose", m, l, &nobr21, &c_b29, 
			    ...
			    ldrwrk, &upd, &r__[jd * r_dim1 + 1], ldr);
		    dgemm("Transpose", "NoTranspose", m, l, &ns, &c_b29, &u[
...
/* L220: */
			...
			dger(m, l, &c_b29, &u[ns + i__ - 1 + u_dim1], ldu, &
				y[ns + j - 1 + y_dim1], ldy, &r__[id + jd * 
				r_dim1], ldr);
			dger(m, l, &c_b24, &dwork[i__ - 1], &ldrwrk, &dwork[
...
/*              for MOESP algorithm. */
		...
		for (j = mmnobr + 1; j <= i__1; ++j) {
		    dswap(&mnobr, &r__[j * r_dim1 + 1], &c__1, &r__[mnobr + 
			    1 + j * r_dim1], &c__1);
/* L250: */
		}
...
/*        Let  Gyy(i,j) = Gyy0(i,j) + y_i*y_i' + y(i+1)*y(i+1)' + ... + */
/*                                    y(i+NS-1)*y(i+NS-1)', */
...
/*           backward order. */
	    ...
	    for (i__ = nr - *l; i__ >= i__1; --i__) {
		i__2 = i__ - mmnobr;
		daxpy(&i__2, &c_b24, &r__[jd + i__ * r_dim1], &c__1, &r__[jd 
			+ *l + (*l + i__) * r_dim1], &c__1);
/* L260: */
	    }
...
/*        Compute/update  Gyy(1,1). */
	...
	if (! first && connec) {
	    dsyrk("Upper", "Transpose", l, &nobr21, &c_b29, &dwork[ldrwrk * *
		    m + 1], &ldrwrk, &upd, &r__[jd + jd * r_dim1], ldr);
	}
	dsyrk("Upper", "Transpose", l, &ns, &c_b29, &y[y_offset], ldy, &upd, 
		&r__[jd + jd * r_dim1], ldr);
...
/*              Compute/update  Gyy(1,j). */
		...
		dgemm("Transpose", "NoTranspose", l, l, &ns, &c_b29, &y[
			y_offset], ldy, &y[j + y_dim1], ldy, &upd, &r__[
			mmnobr + 1 + jd * r_dim1], ldr)
			;
...
/*              structure. */
		...
		if (first) {
		    ...
		    for (i__ = jd - *l; i__ <= i__2; ++i__) {
			i__3 = i__ - mmnobr;
			dcopy(&i__3, &r__[mmnobr + 1 + i__ * r_dim1], &c__1, 
...
/* L270: */
		    ...
		    for (i__ = jd - *l; i__ <= i__2; ++i__) {
			i__3 = i__ - mmnobr;
			daxpy(&i__3, &c_b29, &r__[mmnobr + 1 + i__ * r_dim1],
...
/* L280: */
		...
		i__2 = j - 1;
		for (i__ = 2; i__ <= i__2; ++i__) {
		    dger(l, l, &c_b29, &y[ns + i__ - 1 + y_dim1], ldy, &y[ns 
			    + j - 1 + y_dim1], ldy, &r__[id + jd * r_dim1], 
			    ldr);
		    dger(l, l, &c_b24, &y[i__ - 1 + y_dim1], ldy, &y[j - 1 + 
...
/* L290: */
		...
		i__2 = *l;
		for (i__ = 1; i__ <= i__2; ++i__) {
		    daxpy(&i__, &y[ns + j - 1 + i__ * y_dim1], &y[ns + j - 1 
			    + y_dim1], ldy, &r__[jd + (jd + i__ - 1) * r_dim1]
			    , &c__1);
		    d__1 = -y[j - 1 + i__ * y_dim1];
		    daxpy(&i__, &d__1, &y[j - 1 + y_dim1], ldy, &r__[jd + (
			    jd + i__ - 1) * r_dim1], &c__1);
/* L300: */
		}
...
/*              connected blocks. */
		...
		dgemm("Transpose", "NoTranspose", l, l, &nobr21, &c_b29, &
...
/* L330: */
		...
		for (i__ = 2; i__ <= i__2; ++i__) {
		    dger(l, l, &c_b29, &y[ns + i__ - 1 + y_dim1], ldy, &y[ns 
			    + j - 1 + y_dim1], ldy, &r__[id + jd * r_dim1], 
			    ldr);
		    dger(l, l, &c_b24, &dwork[ldrwrk * *m + i__ - 1], &
...
/* L340: */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    ...
		    daxpy(&i__, &d__1, &dwork[ldrwrk * *m + j - 1], &ldrwrk, 
...
/*              in the first  (M+L)*(2*NOBR-1)  locations of  DWORK. */
		...
		if (*m > 0) {
		    dlacpy("Full", &nobr21, m, &u[ns + 1 + u_dim1], ldu, &
			    dwork[1], &nobr21);
		}
		dlacpy("Full", &nobr21, l, &y[ns + 1 + y_dim1], ldy, &dwork[
			mmnobr - *m + 1], &nobr21);
	    }
...
/*           matrix. */
	    ...
	    dpotrf("Upper", &nr, &r__[r_offset], ldr, &ierr);
	    goto L370;
	}
    } else if (fqralg) {
...
/*        input-output data correlation matrix. */
	...
	ib01my(meth, batch, conct, nobr, m, l, nsmp, &u[u_offset], ldu, &y[
...
/*        input-output data correlation matrix. */
	...
	if (onebch) {
	    ...
	    *iwarn = 2;
	    minwrk = nr << 1;
	    maxwrk = nr + nr * ilaenv(&c__1, "DGEQRF", " ", &ns, &nr, &c_n1, 
...
/* L390: */
	    ...
	    if (moesp) {
		dlacpy("Full", &nobrm1, m, &dwork[inu + *nobr], &ldrwrk, &
			dwork[1], &ldrwrk);
	    }
	}
...
/*              Put the input-output data in the array  R. */
		...
		if (*m > 0) {
			...
			i__1 = *nobr;
			for (i__ = 1; i__ <= i__1; ++i__) {
			    dlacpy("Full", &ns, m, &u[*nobr + i__ + u_dim1], 
...
/* L400: */
			...
			i__1 = *nobr;
			for (i__ = 1; i__ <= i__1; ++i__) {
			    dlacpy("Full", &ns, m, &u[i__ + u_dim1], ldu, &
...
/* L420: */
		...
		i__1 = nobr2;
		for (i__ = 1; i__ <= i__1; ++i__) {
		    dlacpy("Full", &ns, l, &y[i__ + y_dim1], ldy, &r__[(
...
/*                         prefer 2*(M+L)*NOBR+2*(M+L)*NOBR*NB. */
		...
		jwork = itau + nr;
		i__1 = *ldwork - jwork + 1;
		dgeqrf(&ns, &nr, &r__[r_offset], ldr, &dwork[itau], &dwork[
			jwork], &i__1, &ierr);
	    } else {
...
/* L460: */
		...
		i__1 = nobr2;
		for (i__ = 1; i__ <= i__1; ++i__) {
		    dlacpy("Full", &ns, l, &y[i__ + y_dim1], ldy, &dwork[
			    ishfty], &ldrwrk);
		    ishfty += lldrw;
/* L470: */
		}
...
/*                            LDRWRK = min(NS, LDWORK/(2*(M+L)*NOBR)-2). */
		...
		jwork = itau + nr;
		i__1 = *ldwork - jwork + 1;
		dgeqrf(&ns, &nr, &dwork[1], &ldrwrk, &dwork[itau], &dwork[
			jwork], &i__1, &ierr);
		i__1 = min(ns,nr);
		dlacpy("Upper ", &i__1, &nr, &dwork[1], &ldrwrk, &r__[
			r_offset], ldr);
	    }
		...
		i__1 = nr - ns;
		i__2 = nr - ns;
		dlaset("Upper ", &i__1, &i__2, &c_b207, &c_b207, &r__[ns + 1 
			+ (ns + 1) * r_dim1], ldr);
	    }
	    initi += ns;
	}
...
/*              Put the input-output data in the array  DWORK. */
		...
		if (*m > 0) {
			...
			i__2 = *nobr;
			for (i__ = 1; i__ <= i__2; ++i__) {
			    dlacpy("Full", &nsf, m, &u[initi + *nobr + i__ + 
...
/* L480: */
			...
			i__2 = *nobr;
			for (i__ = 1; i__ <= i__2; ++i__) {
			    dlacpy("Full", &nsf, m, &u[initi + i__ + u_dim1],
				     ldu, &dwork[ishft2], &ldrwrk);
			    ishft2 += mldrw;
/* L490: */
			}
...
/* L500: */
		...
		i__2 = nobr2;
		for (i__ = 1; i__ <= i__2; ++i__) {
		    dlacpy("Full", &nsf, l, &y[initi + i__ + y_dim1], ldy, &
			    dwork[ishfty], &ldrwrk);
		    ishfty += lldrw;
/* L510: */
		}
...
/*                 in sequential processing. */
		    ...
		    if (moesp && *m > 0) {
			dlacpy("Full", nobr, m, &u[u_offset], ldu, &dwork[*
				nobr], &ldrwrk);
		    }
...
/*                 block of data in sequential processing. */
			...
			if (moesp) {
			    ...
			    i__2 = nobrm1;
			    for (i__ = 1; i__ <= i__2; ++i__) {
				dlacpy("Full", &nobr21, m, &dwork[ishftu - 
...
/* L520: */
			    ...
			    i__2 = nobrm1;
			    for (i__ = 1; i__ <= i__2; ++i__) {
				dlacpy("Full", &nobr21, m, &dwork[ishft2 - 
...
/* L540: */
		    ...
		    i__2 = nobr21;
		    for (i__ = 1; i__ <= i__2; ++i__) {
			dlacpy("Full", &nobr21, l, &dwork[ishfty - lldrw + 1]
				, &ldrwrk, &dwork[ishfty], &ldrwrk)
				;
			ishfty += lldrw;
/* L550: */
		    }
...
/*              Workspace: need LDRWRK*2*(M+L)*NOBR + 4*(M+L)*NOBR. */
		...
		mb04od("Full", &nr, &c__0, &nsl, &r__[r_offset], ldr, &dwork[
...
/*              in the first  (M+L)*(2*NOBR-1)  locations of  DWORK. */
		...
		if (*m > 0) {
		    dlacpy("Full", &nobr21, m, &u[initi + 1 + u_dim1], ldu, &
			    dwork[1], &nobr21);
		}
		dlacpy("Full", &nobr21, l, &y[initi + 1 + y_dim1], ldy, &
