/* SB03QY.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int sb03qy(char *job, char *trana, char *lyapun, integer *n,
...
/*              Z = Y or Z = W. */
		...
		mb01ru(uplo, "No transpose", n, n, &c_b21, &c_b22, &dwork[1],
			 n, &u[u_offset], ldu, &dwork[1], n, &dwork[itmp], &nn, &info2);
		i__1 = *n + 1;
		dscal(n, &c_b23, &dwork[1], &i__1);
...
/*           Compute RHS = op(W)'*X + X*op(W). */
	    ...
	    dsyr2k(uplo, tranat, n, n, &c_b22, &dwork[1], n, &x[x_offset], 
		    ldx, &c_b21, &dwork[itmp], n);
