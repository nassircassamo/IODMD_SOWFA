/* IB01MY.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ib01my(char *meth, char *batch, char *conct, integer *
...
/*         J. Comput. Appl. Math., 2000 (submitted). */
...
/*     V. Sima, July 2000, August 2000, Feb. 2004. */
...
/*        Workspace: need   (M+L)*2*NOBR*(M+L+3). */
	...
	for (i__ = 2; i__ <= i__1; ++i__) {
	    irev -= nobr2;
	    icol -= ldrwrk;
	    dcopy(&nobr2, &dwork[irev], &c__1, &dwork[icol], &c__1);
/* L10: */
	}

	if (*m > 0) {
	    dlacpy("Full", &nobr2, m, &u[u_offset], ldu, &dwork[iconn + 
		    nobr2], &ldrwrk);
	}
	dlacpy("Full", &nobr2, l, &y[y_offset], ldy, &dwork[iconn + ldrwrk * 
		*m + nobr2], &ldrwrk);
    }
...
/*        Compute/update  Guu(1,1). */
	...
	if (! first && connec) {
	    dsyrk("Upper", "Transpose", m, &nobr2, &c_b18, &dwork[iconn], &
		    ldrwrk, &upd, &dwork[ipg], &nrg);
	}
	dsyrk("Upper", "Transpose", m, &nsm, &c_b18, &u[u_offset], ldu, &upd,
		 &dwork[ipg], &nrg);
	ma02ed("Upper", m, &dwork[ipg], &nrg);
...
/*              Compute/update  Guu(1,j). */
		...
		dgemm("Transpose", "NoTranspose", m, m, &nsm, &c_b18, &u[
...
/*              with connected blocks. */
		...
		dgemm("Transpose", "NoTranspose", m, m, &nobr2, &c_b18, &
			dwork[iconn], &ldrwrk, &dwork[iconn + j - 1], &ldrwrk,
			 &upd, &dwork[ipg + (jd - 1) * nrg], &nrg);
		dgemm("Transpose", "NoTranspose", m, m, &nsm, &c_b18, &u[
...
/*              Compute/update  Guy(1,j). */
		...
		dgemm("Transpose", "NoTranspose", m, l, &nsm, &c_b18, &u[
...
/*              with connected blocks. */
		...
		dgemm("Transpose", "NoTranspose", m, l, &nobr2, &c_b18, &
...
/*        "positive" generators. */
	...
	i__1 = *l;
	for (j = 1; j <= i__1; ++j) {
	    dcopy(m, &dwork[ipg + (mmnobr + j - 1) * nrg], &c__1, &dwork[ipy 
		    + j - 1], &nrg);
/* L60: */
	}
...
/*              Compute/update  Gyu(1,j). */
		...
		dgemm("Transpose", "NoTranspose", l, m, &nsm, &c_b18, &y[
...
/*              with connected blocks. */
		...
		dgemm("Transpose", "NoTranspose", l, m, &nobr2, &c_b18, &
...
/*     Compute/update  Gyy(1,1). */
    ...
    if (! first && connec) {
	dsyrk("Upper", "Transpose", l, &nobr2, &c_b18, &dwork[iconn + ldrwrk 
		* *m], &ldrwrk, &upd, &dwork[ipy + mmnobr * nrg], &nrg);
    }
    dsyrk("Upper", "Transpose", l, &nsm, &c_b18, &y[y_offset], ldy, &upd, &
	    dwork[ipy + mmnobr * nrg], &nrg);
    ma02ed("Upper", l, &dwork[ipy + mmnobr * nrg], &nrg);
...
/*           Compute/update  Gyy(1,j). */
	    ...
	    dgemm("Transpose", "NoTranspose", l, l, &nsm, &c_b18, &y[
...
/*           connected blocks. */
	    ...
	    dgemm("Transpose", "NoTranspose", l, l, &nobr2, &c_b18, &dwork[
...
/*           These will be used to construct the last negative generator. */
	    ...
	    jd = nrg;
	    if (*m > 0) {
		dcopy(m, dum, &c__0, &dwork[jd], &nrg);

		i__1 = nobr21;
		for (j = 1; j <= i__1; ++j) {
		    jd += mnrg;
		    dcopy(m, &u[j + u_dim1], ldu, &dwork[jd], &nrg);
/* L110: */
		}

		jd += mnrg;
	    }
	    dcopy(l, dum, &c__0, &dwork[jd], &nrg);

	    i__1 = nobr21;
	    for (j = 1; j <= i__1; ++j) {
		jd += lnrg;
		dcopy(l, &y[j + y_dim1], ldy, &dwork[jd], &nrg);
/* L120: */
	    }
...
/*           in (M+L)*2*NOBR locations of  DWORK  starting at ICONN. */
	    ...
	    if (*m > 0) {
		dlacpy("Full", &nobr2, m, &u[ns + u_dim1], ldu, &dwork[iconn]
			, &nobr2);
	    }
	    dlacpy("Full", &nobr2, l, &y[ns + y_dim1], ldy, &dwork[iconn + 
		    mmnobr], &nobr2);
	}
...
/*        Workspace: need   (M+L)*4*NOBR*(M+L+1)+M+L. */
	...
	jwork = (nrg << 1) * nr + 1;
	i__1 = nrg + 1;
	dcopy(m, &dwork[ipg], &i__1, &dwork[jwork], &c__1);
	i__1 = nrg + 1;
	dcopy(l, &dwork[ipy + mmnobr * nrg], &i__1, &dwork[jwork + *m], &
		c__1);

	i__1 = *m + *l;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    i__2 = *m + *l;
	    iwork[i__] = idamax(&i__2, &dwork[jwork], &c__1);
...
/*              Error exit. */
		...
		*info = 1;
	    ...
	    d__1 = 1. / beta;
	    dscal(&nr, &d__1, &dwork[ipg + imax - 1], &nrg);
	    dcopy(&nr, &dwork[ipg + imax - 1], &nrg, &dwork[ing + imax - 1], 
		    &nrg);
...
/*           generator. */
	    ...
	    dcopy(&nr, &dwork[nrg], &nrg, &dwork[ing + nrg - 1], &nrg);
	}
...
/*                   prefer (M+L)*4*NOBR*(M+L+1)+M+M*NB. */
	    ...
	    ingc = ing;
	    i__1 = *ldwork - jwork + 1;
	    dgeqrf(&nrg, m, &dwork[ipg], &nrg, &dwork[itau], &dwork[jwork], &
		    i__1, &ierr);
/* Computing MAX */
...
/*                                                 ((M+L)*2*NOBR-M)*NB. */
	    ...
	    i__1 = nr - *m;
	    i__2 = *ldwork - jwork + 1;
	    dormqr("Left", "Transpose", &nrg, &i__1, m, &dwork[ipg], &nrg, &
...
/*           array. */
	    ...
	    i__1 = *m;
	    for (j = 1; j <= i__1; ++j) {
		dlarfg(&nrg, &dwork[ingc], &dwork[ingc + 1], &c__1, &tau);
		beta = dwork[ingc];
		dwork[ingc] = 1.;
		ingp = ingc + nrg;
		i__2 = nr - j;
		dlarf("Left", &nrg, &i__2, &dwork[ingc], &c__1, &tau, &dwork[
			ingp], &nrg, &dwork[itau]);
		dwork[ingc] = beta;
...
/*              rotation. */
		...
		ma02fd(&dwork[ipg + (j - 1) * (nrg + 1)], &dwork[ingc], &cs, 
			&sn, &ierr);
		if (ierr != 0) {
...
/*           calculation of the following row. */
	    ...
	    dlacpy("Upper", m, &nr, &dwork[ipg], &nrg, &r__[r_offset], ldr);

	    i__1 = mnrg;
	    i__3 = -mnrg;
	    for (i__ = (mmnobr - *m) * nrg; i__3 < 0 ? i__ >= i__1 : i__ <= 
		    i__1; i__ += i__3) {
		dlacpy("Full", m, m, &dwork[ipg + i__ - mnrg], &nrg, &dwork[
			ipg + i__], &nrg);
/* L220: */
	    }
	    ...
	    for (i__ = (nr - *l) * nrg; i__1 < 0 ? i__ >= i__3 : i__ <= i__3; 
		    i__ += i__1) {
		dlacpy("Full", m, l, &dwork[ipg + i__ - lnrg], &nrg, &dwork[
			ipg + i__], &nrg);
/* L230: */
	    }

	    dlaset("Full", m, l, &c_b111, &c_b111, &dwork[ipgc], &nrg);
...
/*           Workspace: need   (M+L)*4*NOBR*(M+L+1)+2*NOBR*(M+L)-M. */
		...
		i__3 = nr - icol - *m;
		i__2 = *l + 1;
		mb04od("Full", m, &i__3, &i__2, &dwork[ipg + jds], &nrg, &
		...
		for (j = 1; j <= i__3; ++j) {
		    ...
		    ingp = ingc + nrg;
		    i__2 = nr - icj;
		    dlarf("Left", &nrg, &i__2, &dwork[ingc], &c__1, &tau, &
...
/*                 rotation. */
		    ...
		    ma02fd(&dwork[ipg + j - 1 + (icj - 1) * nrg], &dwork[
			    ingc], &cs, &sn, &ierr);
		    if (ierr != 0) {
...
/*              calculation of the following row. */
		...
		i__3 = nr - icol;
		dlacpy("Upper", m, &i__3, &dwork[ipg + jds], &nrg, &r__[icol 
			+ 1 + (icol + 1) * r_dim1], ldr);
		icol += *m;
		...
		for (i__ = (mmnobr - *m) * nrg; i__4 < 0 ? i__ >= i__3 : i__ 
			<= i__3; i__ += i__4) {
		    dlacpy("Full", m, m, &dwork[ipg + i__ - mnrg], &nrg, &
			    dwork[ipg + i__], &nrg);
/* L260: */
		}
		...
		for (i__ = (nr - *l) * nrg; i__3 < 0 ? i__ >= i__4 : i__ <= 
			i__4; i__ += i__3) {
		    dlacpy("Full", m, l, &dwork[ipg + i__ - lnrg], &nrg, &
			    dwork[ipg + i__], &nrg);
/* L270: */
		}
...
/*                   prefer (M+L)*4*NOBR*(M+L+1)+L+L*NB. */
	...
	ingc = ing + mmnobr * nrg;
	i__1 = *ldwork - jwork + 1;
	dgeqrf(&nrg, l, &dwork[ipgc], &nrg, &dwork[itau], &dwork[jwork], &
		i__1, &ierr);
/* Computing MAX */
...
/*                   prefer (M+L)*4*NOBR*(M+L+1)+L+(L*2*NOBR-L)*NB. */
	...
	i__1 = llnobr - *l;
	i__3 = *ldwork - jwork + 1;
	dormqr("Left", "Transpose", &nrg, &i__1, l, &dwork[ipgc], &nrg, &
...
/*        Householder transformations and modified hyperbolic rotations. */
	...
	for (j = 1; j <= i__1; ++j) {
	    ...
	    ingp = ingc + nrg;
	    i__3 = llnobr - j;
	    dlarf("Left", &nrg, &i__3, &dwork[ingc], &c__1, &tau, &dwork[
		    ingp], &nrg, &dwork[itau]);
	    dwork[ingc] = beta;
...
/*           rotation. */
	    ...
	    ma02fd(&dwork[ipgc + (j - 1) * (nrg + 1)], &dwork[ingc], &cs, &
		    sn, &ierr);
	    if (ierr != 0) {
...
/*        calculation of the following row. */
	...
	dlacpy("Upper", l, &llnobr, &dwork[ipgc], &nrg, &r__[mmnobr + 1 + (
		mmnobr + 1) * r_dim1], ldr);
	...
	for (i__ = (nr - *l) * nrg; i__4 < 0 ? i__ >= i__1 : i__ <= i__1; i__ 
		+= i__4) {
	    dlacpy("Full", l, l, &dwork[ipg + i__ - lnrg], &nrg, &dwork[ipg 
		    + i__], &nrg);
/* L310: */
	}
...
/*        Workspace: need   (M+L)*4*NOBR*(M+L+1)+2*NOBR*L-L. */
	...
	for (k = 2; k <= i__4; ++k) {
	    i__1 = llnobr - icol - *l;
	    i__3 = *m + 1;
	    mb04od("Full", l, &i__1, &i__3, &dwork[ipgc + jds], &nrg, &dwork[
...
/*              rotation. */
		...
		ma02fd(&dwork[ipgc + j - 1 + (icj - 1) * nrg], &dwork[ingc], 
			&cs, &sn, &ierr);
		if (ierr != 0) {
...
/*           calculation of the following row. */
	    ...
	    i__1 = llnobr - icol;
	    dlacpy("Upper", l, &i__1, &dwork[ipgc + jds], &nrg, &r__[mmnobr 
		    + icol + 1 + (mmnobr + icol + 1) * r_dim1], ldr);

	    i__1 = (mmnobr + icol) * nrg;
	    i__2 = -lnrg;
	    for (i__ = (nr - *l) * nrg; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; 
		    i__ += i__2) {
		dlacpy("Full", l, l, &dwork[ipg + i__ - lnrg], &nrg, &dwork[
			ipg + i__], &nrg);
/* L340: */
	    }
...
/*           more efficient. */
	    ...
	    i__4 = mnobr;
	    for (j = 1; j <= i__4; ++j) {
		dswap(&j, &r__[j * r_dim1 + 1], &c__1, &r__[(mnobr + j) * 
			r_dim1 + 1], &c__1);
		dcopy(&mnobr, &r__[j + 1 + (mnobr + j) * r_dim1], &c__1, &
			r__[j + 1 + j * r_dim1], &c__1);
		i__2 = mmnobr - j;
		dcopy(&i__2, dum, &c__0, &r__[j + 1 + (mnobr + j) * r_dim1], 
			&c__1);
/* L360: */
	    }
...
/*           Workspace: need 2*(M+L)*NOBR. */
	    ...
	    i__4 = mnobr - 1;
	    i__2 = *ldwork - jwork + 1;
	    mb04id(&mmnobr, &mmnobr, &i__4, &llnobr, &r__[r_offset], ldr, &
