/* IB01BD.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ib01bd(char *meth, char *job, char *jobck, integer *
...
/*     March 2000, August 2000, Sept. 2001. */
...
/*     Call the computational routine for estimating system matrices. */
    ...
    if (! combin) {
	i__1 = *ldwork - jwork + 1;
	ib01pd(meth, job, jobcv, nobr, n, m, l, nsmpl, &r__[r_offset], ldr, &
	...
	if (withc) {
		...
		*(unsigned char *)jobcov = *(unsigned char *)jobcv;
	    ...
	    i__1 = *ldwork - jwork + 1;
	    ib01pd("MOESP", "C and A", jobcov, nobr, n, m, l, nsmpl, &r__[
...
/* Computing MAX */
	...
	if (withb) {
		...
		*(unsigned char *)jobbd = 'D';
	    ...
	    i__1 = *ldwork - jwork + 1;
	    ib01pd("N4SID", jobbd, jobcv, nobr, n, m, l, nsmpl, &r__[
		    r_offset], ldr, &a[a_offset], lda, &c__[c_offset], ldc, &
		    b[b_offset], ldb, &d__[d_offset], ldd, &q[q_offset], ldq, 
		    &ry[ry_offset], ldry, &s[s_offset], lds, &dwork[io], &
		    lnobr, tol, &iwork[1], &dwork[jwork], &i__1, &iwarnl, 
...
/*           gain to zero, set accuracy parameters and return. */
	    ...
	    dlaset("Full", n, l, &c_b21, &c_b21, &k[k_offset], ldk);
...
/*                       prefer larger. */
	    ...
	    ma02ad("Full", n, n, &a[a_offset], lda, &dwork[ia], n);
	    ma02ad("Full", l, n, &c__[c_offset], ldc, &dwork[ic], n);
	    dlacpy("Upper", n, n, &q[q_offset], ldq, &dwork[iq], n);
	    dlacpy("Upper", l, l, &ry[ry_offset], ldry, &dwork[ir], l);
	    dlacpy("Full", n, l, &s[s_offset], lds, &dwork[is], n);

	    i__1 = *ldwork - jwork + 1;
	    sb02mt("G needed", "Nonzero S", "Not factored", "Upper", n, l, &
		    dwork[ia], n, &dwork[ic], n, &dwork[iq], n, &dwork[ir], l,
		     &dwork[is], n, &iwork[1], &ifact, &dwork[ig], n, &iwork[*
		    l + 1], &dwork[jwork], &i__1, &ierr);
	    if (ierr != 0) {
		*info = 3;
		return 0;
	    }
...
/*                       prefer larger. */
	    ...
	    i__1 = *ldwork - jwork + 1;
	    sb02rd("All", "Discrete", "Direct", "NoTranspose", "Upper", 
		    ...
		    iwork[1], &dwork[jwork], &i__1, &bwork[1], &ierr);
...
/*                       prefer larger. */
	    ...
	    i__1 = *ldwork - jwork + 1;
	    sb02nd("Discrete", "NotFactored", "Upper", "Nonzero S", n, l, &
...
/* Computing MAX */
	    ...
	    ma02ad("Full", l, n, &dwork[ik], l, &k[k_offset], ldk)
		    ;
