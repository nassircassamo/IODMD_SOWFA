/* SB02ND.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int sb02nd(char *dico, char *fact, char *uplo, char *jobl, 
...
/*     Test the input scalar arguments. */
	...
	*info = -1;
    } else if (! lfacta && ! lsame(fact, "N") || discr 
	    && lfactu) {
	*info = -2;
    } else if (! luplou && ! lsame(uplo, "L")) {
	*info = -3;
    } else if (! withl && ! lsame(jobl, "Z")) {
	*info = -4;
    } else if (*n < 0) {
	*info = -5;
    } else if (*m < 0) {
...
/*     following subroutine, as returned by ILAENV.) */
    ...
    dgemm("Transpose", "No transpose", m, n, n, &c_b16, &b[b_offset], ldb, &
	    x[x_offset], ldx, &c_b17, &f[f_offset], ldf);
...
/*           Discrete-time case with R not factored. Compute R + B'XB. */
		...
		i__1 = *m;
		for (j = 1; j <= i__1; ++j) {
		    dgemv("No transpose", &j, n, &c_b16, &f[f_offset], ldf, &
...
/* L10: */
		...
		i__1 = *m;
		for (j = 1; j <= i__1; ++j) {
		    dgemv("Transpose", n, &j, &c_b16, &b[b_offset], ldb, &f[
...
/*        Workspace: need M. */
	...
	rnormp = dlansy("1-norm", uplo, m, &r__[r_offset], ldr, &dwork[1]);
	wrkopt = max(wrkopt,*m);
    }
...
/*        Workspace: need N. */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    dcopy(n, &f[i__ + f_dim1], ldf, &dwork[1], &c__1);
	    dgemv("Transpose", n, n, &c_b16, &a[a_offset], lda, &dwork[1], &
		    c__1, &c_b17, &f[i__ + f_dim1], ldf);
/* L30: */
	}
...
/*                      prefer min(P,M) + M*NB. */
	    ...
	    jwork = itau + min(*p,*m);
	    i__1 = *ldwork - jwork + 1;
	    dgeqrf(p, m, &r__[r_offset], ldr, &dwork[itau], &dwork[jwork], &
		    i__1, &ifail);
/* Computing MAX */
...
/* L60: */
		...
		if (! luplou) {
		    i__2 = i__ - 1;
		    dcopy(&i__2, &r__[i__ * r_dim1 + 1], &c__1, &r__[i__ + 
			    r_dim1], ldr);
		}
/* L70: */
	    }
	    ...
	    if (*p < *m) {
		i__1 = *m - *p;
		dlaset("Full", &i__1, m, &c_b17, &c_b17, &r__[*p + 1 + 
			r_dim1], ldr);
		if (! discr) {
		    dwork[2] = 0.;
		    *info = *m + 1;
...
/*                      JW = N+3 for spectral factorization of X. */
	    ...
	    i__1 = *ldx + 1;
	    dcopy(n, &x[x_offset], &i__1, &dwork[1], &c__1);
	    dpotrf("Upper", n, &x[x_offset], ldx, &ifail);
	    if (ifail == 0) {

/*              Use Cholesky factorization of X to compute chol(X)*B. */

		oufact[2] = 1;
		dtrmm("Left", "Upper", "No transpose", "Non unit", n, m, &
			c_b16, &x[x_offset], ldx, &b[b_offset], ldb);
	    } else {
...
/*                         prefer N*(NB+2)+N+2. */
		...
		oufact[2] = 2;
		i__1 = *ldx + 1;
		dcopy(n, &dwork[1], &c__1, &x[x_offset], &i__1);
		i__1 = *ldwork - jw + 1;
		dsyev("Vectors", "Lower", n, &x[x_offset], ldx, &dwork[3], &
			dwork[jw], &i__1, &ifail);
		if (ifail > 0) {
		    *info = *m + 2;
		    return 0;
...
L80:
		...
		i__1 = *m;
		for (j = 1; j <= i__1; ++j) {
		    dcopy(n, &b[j * b_dim1 + 1], &c__1, &dwork[jw], &c__1);
		    dgemv("Transpose", n, n, &c_b16, &x[x_offset], ldx, &
...
/* L90: */
		...
		for (i__ = jz + 1; i__ <= i__1; ++i__) {
		    ...
		    dscal(m, &d__2, &b[i__ + b_dim1], ldb);
/* L100: */
		}

		if (jz > 0) {
		    dlaset("Full", &jz, m, &c_b17, &c_b17, &b[b_offset], ldb);
		}
	    }
...
/*              For efficiency, use the transposed of the lower triangle. */
		...
		for (i__ = 2; i__ <= i__1; ++i__) {
		    i__2 = i__ - 1;
		    dcopy(&i__2, &r__[i__ + r_dim1], ldr, &r__[i__ * r_dim1 
			    + 1], &c__1);
/* L110: */
		}
...
/*           Workspace: need JW+2*M-1. */
	    ...
	    mb04kd("Full", m, &c__0, n, &r__[r_offset], ldr, &b[b_offset], 
...
/*           Workspace: need JW+3*M-1. */
	    ...
	    dtrcon("1-norm", uplo, "Non unit", m, &r__[r_offset], ldr, &
		    rcond, &dwork[jw], &iwork[1], &ifail);
	    oufact[1] = 1;
/* Computing MAX */
...
/*           Workspace: need 3*M. */
	    ...
	    dpocon(uplo, m, &r__[r_offset], ldr, &rnormp, &rcond, &dwork[1], 
		    &iwork[1], info);
...
/*           Workspace: need   2*M. */
	    ...
	    dsycon(uplo, m, &r__[r_offset], ldr, &ipiv[1], &rnormp, &rcond, &
		    dwork[1], &iwork[1], info);
...
/*        Solve the positive definite linear system. */
	...
	dpotrs(uplo, m, n, &r__[r_offset], ldr, &f[f_offset], ldf, info);
    } else {

/*        Solve the indefinite linear system. */

	dsytrs(uplo, m, n, &r__[r_offset], ldr, &ipiv[1], &f[f_offset], ldf, info);
    }
