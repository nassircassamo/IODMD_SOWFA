/* IB01QD.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ib01qd(char *jobx0, char *job, integer *n, integer *m, 
...
/* Computing MAX */
	    i__3 = *nsmp - *m;
	    i__1 = *m * ilaenv(&c__1, "DGEQRF", " ", nsmp, m, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1), i__2 = ncol + ncol * ilaenv(&c__1, 
		    "DGEQRF", " ", &i__3, &ncol, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1);
	    maxwrk = isize + *n + *m + max(i__1,i__2);
/* Computing MAX */
/* Computing MAX */
	    i__5 = *nsmp - *m;
	    i__3 = ncp1 * ilaenv(&c__1, "DORMQR", "LT", nsmp, &ncp1, m, &
		    c_n1, (ftnlen)6, (ftnlen)2), i__4 = ncol + ilaenv(&c__1, 
		    "DORMQR", "LT", &i__5, &c__1, &ncol, &c_n1, (ftnlen)6, (ftnlen)2);
	    i__1 = maxwrk, i__2 = isize + *n + *m + max(i__3,i__4);
	    maxwrk = max(i__1,i__2);
	} else {
/* Computing MAX */
	    i__1 = ncol * ilaenv(&c__1, "DGEQRF", " ", &nsmpl, &ncol, &c_n1, 
		    &c_n1, (ftnlen)6, (ftnlen)1), i__2 = ilaenv(&c__1, "DORMQR",
            "LT", &nsmpl, &c__1, &ncol, &c_n1, (ftnlen)6, (ftnlen)2);
...
/*              TF01ND is used. */
		...
		if (first) {
		    dcopy(n, dum, &c__0, &dwork[ixsave], &c__1);
		}
		dcopy(n, &dwork[ixsave], &c__1, &x0[1], &c__1);
		ini = iy;

		i__4 = nobs;
		for (k = 1; k <= i__4; ++k) {
		    dgemv("No transpose", l, n, &c_b45, &c__[c_offset], ldc, 
			    &x0[1], &c__1, &c_b47, &dwork[iy], &nobs);
		    ++iy;
...
/* L10: */
		    ...
		    dcopy(n, &x0[1], &c__1, &dwork[ixsave], &c__1);
/* L20: */
		}
...
/*                            c = L*N, if NCYCLE > 1. */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    dcopy(l, &c__[i__ * c_dim1 + 1], &c__1, &dwork[ig], &
			    nobs);
		    ig += lddw;
/* L50: */
		}
		...
		for (i__ = ic; i__3 < 0 ? i__ >= i__2 : i__ <= i__2; i__ += 
			i__3) {
		    dcopy(l, &dwork[i__], &c__1, &dwork[ig], &nobs);
		    ig += lddw;
/* L60: */
		}
...
/*           Use powers of the matrix  A:  A ,  p = 2**(J-1). */
		...
		i__2 = *lda + 1;
		i__4 = *n + 1;
		dcopy(&i__3, &a[a_dim1 + 2], &i__2, &dwork[ia + 1], &i__4);
	    }
	    i2 = 1;
	    nrow = 0;
		...
		i__2 = *l;
		for (i__ = 1; i__ <= i__2; ++i__) {
		    dlacpy("Full", &nrow, n, &dwork[igam], &lddw, &dwork[
			    igam + i2], &lddw);
		    dtrmm("Right", "Upper", "No Transpose", "Non Unit", &
...
/*                 to the product. */
		    ...
		    i__4 = *n - 1;
		    for (ix = 1; ix <= i__4; ++ix) {
			daxpy(&nrow, &dwork[ia + (ix - 1) * *n + ix], &dwork[
...
/* L80: */
		...
		if (j < iexpon) {
			...
			i__4 = *n + 1;
			i__5 = *n + 1;
			dcopy(&i__2, &dwork[ia + 1], &i__4, &dwork[ias + 1], 
				&i__5);
		    }
...
/* L90: */
		...
		for (i__ = ic; i__2 < 0 ? i__ >= i__3 : i__ <= i__3; i__ += 
			i__2) {
		    dcopy(l, &dwork[ig], &nobs, &dwork[i__], &c__1);
		    ig += lddw;
/* L100: */
		}

		dtrmm("Right", "Upper", "No Transpose", "Non Unit", l, n, &
			c_b45, &a[a_offset], lda, &dwork[ic], l);
		ig = igs;
...
/*              product. */
		...
		i__2 = *n - 1;
		for (ix = 1; ix <= i__2; ++ix) {
		    daxpy(l, &a[ix + 1 + ix * a_dim1], &dwork[ig + lddw], &
...
/*        problem. */
	...
	i__2 = *l;
	for (k = 1; k <= i__2; ++k) {
	    dcopy(&nobs, &y[iypnt + k * y_dim1], &c__1, &dwork[iy], &c__1);
	    iy += nobs;
/* L120: */
	}
...
/*                         prefer v + M*NB. */
		...
		i__2 = *ldwork - jwork + 1;
		dgeqrf(&nobs, m, &u[u_offset], ldu, &dwork[itauu], &dwork[
			jwork], &i__2, &ierr);
/*                                                  ij */
...
/*                         prefer v + (r + 1)*NB. */
		...
		for (k = 1; k <= i__2; ++k) {
		    i__3 = *ldwork - jwork + 1;
		    dormqr("Left", "Transpose", &nobs, &ncp1, m, &u[u_offset]
			    , ldu, &dwork[itauu], &dwork[iny + (k - 1) * nobs]
			    , &lddw, &dwork[jwork], &i__3, &ierr);
/* L130: */
		}
...
/*                            prefer v + r + r*NB. */
		    ...
		    i__2 = nobs - *m;
		    i__3 = *ldwork - jwork + 1;
		    dgeqrf(&i__2, &ncol, &dwork[ini], &lddw, &dwork[itau], &
...
/*                            prefer v + r + NB. */
		    ...
		    i__2 = nobs - *m;
		    i__3 = *ldwork - jwork + 1;
		    dormqr("Left", "Transpose", &i__2, &c__1, &ncol, &dwork[
...
/*                 Workspace: need   v + r - 1. */
		    ...
		    for (k = 2; k <= i__2; ++k) {
			i__3 = nobs - *m;
			mb04od("Full", &ncol, &c__1, &i__3, &dwork[ini], &
...
/*                 Workspace: need   v. */
		    ...
		    dlacpy("Upper", &ncol, &ncp1, &dwork[ini], &lddw, &dwork[
			    inir], &ldr);

		    i__2 = *l;
		    for (k = 1; k <= i__2; ++k) {
			dlacpy("Full", m, &ncp1, &dwork[iny + (k - 1) * nobs]
...
/*              Workspace: need   v + r + 1. */
		...
		mb04od("Full", m, &ncp1, &nobs, &u[u_offset], ldu, &u[iupnt 
...
/*              Workspace: need   v + r + 1. */
		...
		for (k = 2; k <= i__2; ++k) {
		    ...
		    i__3 = *m;
		    for (ix = 1; ix <= i__3; ++ix) {
			mb04oy(&nobs, &ncp1, &u[iupnt + ix * u_dim1], &dwork[
...
/*                 Workspace: need   v + r - 1. */
		    ...
		    i__2 = *l;
		    for (k = 1; k <= i__2; ++k) {
			mb04od("Full", &ncol, &c__1, &nobs, &dwork[inir], &
...
/*                         prefer v + r + r*NB. */
		...
		i__2 = *ldwork - jwork + 1;
		dgeqrf(&lddw, &ncol, &dwork[iny], &lddw, &dwork[itau], &
...
/*                         prefer v + r + NB. */
		...
		i__2 = *ldwork - jwork + 1;
		dormqr("Left", "Transpose", &lddw, &c__1, &ncol, &dwork[iny],
...
/*                 Workspace: need   v. */
		    ...
		    dlacpy("Upper", &ncol, &ncp1, &dwork[iny], &lddw, &dwork[
			    inir], &ldr);
		}
	    } else {
...
/*              Workspace: need   v + r - 1. */
		...
		mb04od("Full", &ncol, &c__1, &lnob, &dwork[inir], &ldr, &
...
/*                      u = w,           if NCYCLE > 1. */
    ...
    dtrcon("1-norm", "Upper", "No Transpose", &ncol, &dwork[inir], &ldr, &
	    rcond, &dwork[ie], &iwork[1], &ierr);
...
/*                   prefer larger. */
	...
	if (ncol > 1) {
	    i__1 = ncol - 1;
	    i__2 = ncol - 1;
	    dlaset("Lower", &i__1, &i__2, &c_b47, &c_b47, &dwork[inir + 1], &
		    ldr);
	}
	isv = ie;
	jwork = isv + ncol;
	i__1 = *ldwork - jwork + 1;
	dgelss(&ncol, &ncol, &c__1, &dwork[inir], &ldr, &dwork[inih], &ldr, &
...
/*        Find the least squares solution using QR decomposition only. */
	...
	dtrsm("Left", "Upper", "No Transpose", "Non Unit", &ncol, &c__1, &
		c_b45, &dwork[inir], &ldr, &dwork[inih], &ldr);
    }

/*     Setup the estimated n-by-m input matrix  B,  and the estimated */
/*     initial state of the system  x0. */

    dlacpy("Full", n, m, &dwork[inih], n, &b[b_offset], ldb);

    if (*n > 0 && withx0) {
	dcopy(n, &dwork[inih + nm], &c__1, &x0[1], &c__1);
    } else {
	dcopy(n, dum, &c__0, &x0[1], &c__1);
    }
...
/*        Compute the estimated l-by-m input/output matrix  D. */
	...
	if (ncyc) {
	    irhs = inis + lm * ncol;
	    dgemv("No Transpose", &lm, &ncol, &c_b130, &dwork[inis], &lm, &
		    dwork[inih], &c__1, &c_b45, &dwork[irhs], &c__1);
	} else {

	    i__1 = *l;
	    for (k = 1; k <= i__1; ++k) {
		dgemv("No Transpose", m, &ncol, &c_b130, &dwork[inis + (k - 
			1) * nobs], &lddw, &dwork[inih], &c__1, &c_b45, &
			dwork[irhs + (k - 1) * nobs], &c__1);
/* L200: */
	    }

	    i__1 = *l;
	    for (k = 2; k <= i__1; ++k) {
		dcopy(m, &dwork[irhs + (k - 1) * nobs], &c__1, &dwork[irhs + 
			(k - 1) * *m], &c__1);
/* L210: */
	    }
...
/*        Workspace: need  u + 3*M. */
	...
	dtrcon("1-norm", "Upper", "No Transpose", m, &u[u_offset], ldu, &
		rcondu, &dwork[ie], &iwork[1], &ierr);
...
/*                      prefer larger. */
	    ...
	    isv = iq + *m * *m;
	    jwork = isv + *m;
	    dlacpy("Upper", m, m, &u[u_offset], ldu, &dwork[ie], m);
	    i__1 = *ldwork - jwork + 1;
	    mb02ud("Not Factored", "Left", "No Transpose", "No Pinv", m, l, &
		    c_b45, &toll, &rank, &dwork[ie], m, &dwork[iq], m, &dwork[
		    isv], &dwork[irhs], m, dum, &c__1, &dwork[jwork], &i__1, &ierr);
	    if (ierr > 0) {
...
/* Computing MAX */
	    ...
	    dtrsm("Left", "Upper", "No Transpose", "Non Unit", m, l, &c_b45, 
		    &u[u_offset], ldu, &dwork[irhs], m);
	}
	ma02ad("Full", m, l, &dwork[irhs], m, &d__[d_offset], ldd)
		;
