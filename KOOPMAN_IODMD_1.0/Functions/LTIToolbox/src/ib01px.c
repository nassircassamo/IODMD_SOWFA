/* IB01PX.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ib01px(char *job, integer *nobr, integer *n, integer *m,
...
/*     In the same loop, vectorize K in X. */
    ...
    i__1 = mnobr * npl;
    i__2 = *m * npl;
    dlaset("Full", &i__1, &i__2, &c_b11, &c_b11, &r__[r_offset], ldr);
    i__1 = mnobr - 1;
    i__2 = mnobr - 1;
    dlaset("Lower", &i__1, &i__2, &c_b11, &c_b11, &uf[uf_dim1 + 2], lduf);
    jwork = npl * *l + 1;

    i__1 = *nobr;
    for (i__ = 1; i__ <= i__1; ++i__) {
	dlacpy("Full", &npl, l, &ul[((i__ - 1) * *l + 1) * ul_dim1 + 1], 
		ldul, &dwork[1], &npl);
	if (i__ < *nobr) {
	    i__2 = *l * (*nobr - i__);
	    dgemm("NoTranspose", "NoTranspose", &npl, n, &i__2, &c_b20, &ul[(
		    i__ * *l + 1) * ul_dim1 + 1], ldul, &un[un_offset], ldun, 
		    &c_b11, &dwork[jwork], &npl);
	} else {
	    dlaset("Full", &npl, n, &c_b11, &c_b11, &dwork[jwork], &npl);
	}
	mb01vd("NoTranspose", "NoTranspose", &mnobr, m, &npl, &npl, &c_b20, &
		c_b20, &uf[((i__ - 1) * *m + 1) * uf_dim1 + 1], lduf, &dwork[
		1], &npl, &r__[r_offset], ldr, &mkron, &nkron, &ierr);
	dlacpy("Full", &npl, m, &k[((i__ - 1) * *m + 1) * k_dim1 + 1], ldk, &
		x[(i__ - 1) * nkron + 1], &npl);
/* L60: */
    }
...
/* L70: */
    ...
    dgelsy(&mkron, &nkron, &c__1, &r__[r_offset], ldr, &x[1], &mkron, &iwork[
...
/*     Workspace: need 3*M*(N+L). */
    ...
    dtrcon("1-norm", "Upper", "NonUnit", &nkron, &r__[r_offset], ldr, &rcond,
...
/*     Construct the matrix  D,  if needed. */
    ...
    if (withd) {
	dlacpy("Full", l, m, &x[1], &npl, &d__[d_offset], ldd);
    }

/*     Construct the matrix  B. */

    dlacpy("Full", n, m, &x[lp1], &npl, &b[b_offset], ldb);
