/* MB02UD.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int mb02ud(char *fact, char *side, char *trans, char *jobp, 
...
/*     subroutine, as returned by ILAENV.) */
	...
	if (nfct) {
/* Computing MAX */
	    i__1 = maxwrk, i__2 = l * 3 + (l << 1) * ilaenv(&c__1, "DGEBRD", " ", &l, &l, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1);
	    maxwrk = max(i__1,i__2);
/* Computing MAX */
	    i__1 = maxwrk, i__2 = l * 3 + l * ilaenv(&c__1, "DORGBR", "Q", &l, &l, &l, &c_n1, (ftnlen)6, (ftnlen)1);
	    maxwrk = max(i__1,i__2);
/* Computing MAX */
	    i__1 = maxwrk, i__2 = l * 3 + l * ilaenv(&c__1, "DORGBR", "P", &l, &l, &l, &c_n1, (ftnlen)6, (ftnlen)1);
	    maxwrk = max(i__1,i__2);
/* Computing MAX */
	    i__1 = 1, i__2 = l * 5;
...
/*                   prefer larger. */
	...
	mb03ud("Vectors", "Vectors", &l, &r__[r_offset], ldr, &q[q_offset], 
		ldq, &sv[1], &dwork[1], ldwork, info);
	if (*info != 0) {
	    return 0;
	}
...
/* Computing MAX */
	d__1 = toll * sv[1], d__2 = dlamch("Safe minimum");
	toll = max(d__1,d__2);
...
/*           Compute  pinv(S)'*P'  in R. */
	    ...
	    mb01sd("Row scaling", rank, &l, &r__[r_offset], ldr, &sv[1], &sv[1]);

/*           Compute  pinv(R) = P*pinv(S)*Q'  in  RP. */

	    dgemm("Transpose", "Transpose", &l, &l, rank, &c_b35, &r__[r_offset], 
                ldr, &q[q_offset], ldq, &c_b36, &rp[rp_offset], ldrp);
	}
    }
...
/*     Set X = 0 if alpha = 0. */
    ...
    if (*alpha == 0.) {
	dlaset("Full", m, n, &c_b36, &c_b36, &b[b_offset], ldb);
	dwork[1] = (doublereal) maxwrk;
	return 0;
    }
...
/*                       prefer M*N (BLAS 3). */
	    ...
	    if (*ldwork >= mn) {
		dgemm(trans, "NoTranspose", m, n, m, alpha, &rp[rp_offset], 
			ldrp, &b[b_offset], ldb, &c_b36, &dwork[1], m);
		dlacpy("Full", m, n, &dwork[1], m, &b[b_offset], ldb);
	    } else {

		i__1 = *n;
		for (i__ = 1; i__ <= i__1; ++i__) {
		    dgemv(trans, m, m, alpha, &rp[rp_offset], ldrp, &b[i__ * 
			    b_dim1 + 1], &c__1, &c_b36, &dwork[1], &c__1);
		    dcopy(m, &dwork[1], &c__1, &b[i__ * b_dim1 + 1], &c__1);
/* L40: */
		}
...
/*                       prefer M*N (BLAS 3). */
	    ...
	    if (*ldwork >= mn) {
		dgemm("NoTranspose", trans, m, n, n, alpha, &b[b_offset], 
		...
		i__1 = *m;
		for (i__ = 1; i__ <= i__1; ++i__) {
		    dgemv(ntran, n, n, alpha, &rp[rp_offset], ldrp, &b[i__ + 
			    b_dim1], ldb, &c_b36, &dwork[1], &c__1);
		    dcopy(n, &dwork[1], &c__1, &b[i__ + b_dim1], ldb);
/* L50: */
		}
...
/*                 Compute  alpha*P'*B  in workspace. */
		    ...
		    dgemm("NoTranspose", "NoTranspose", m, n, m, alpha, &r__[
			    r_offset], ldr, &b[b_offset], ldb, &c_b36, &dwork[1], m);

/*                 Compute  alpha*pinv(S)'*P'*B. */

		    mb01sd("Row scaling", rank, n, &dwork[1], m, &sv[1], &sv[1]);

/*                 Compute  alpha*Q*pinv(S)'*P'*B. */

		    dgemm("NoTranspose", "NoTranspose", m, n, rank, &c_b35, &
...
/*                 Compute  alpha*Q'*B  in workspace. */
		    ...
		    dgemm("Transpose", "NoTranspose", m, n, m, alpha, &q[
			    q_offset], ldq, &b[b_offset], ldb, &c_b36, &dwork[1], m);
...
/*                 Compute  alpha*P*pinv(S)*Q'*B. */
		    ...
		    dgemm("Transpose", "NoTranspose", m, n, rank, &c_b35, &
			    r__[r_offset], ldr, &dwork[1], m, &c_b36, &b[b_offset], ldb);
		}
	    } else {
		if (tran) {
...
/*                 Compute  alpha*P'*B  in B using workspace. */
		    ...
		    i__1 = *n;
		    for (i__ = 1; i__ <= i__1; ++i__) {
			dgemv("NoTranspose", m, m, alpha, &r__[r_offset], 
...
/*                 Compute  alpha*pinv(S)'*P'*B. */
		    ...
		    mb01sd("Row scaling", rank, n, &b[b_offset], ldb, &sv[1],&sv[1]);

/*                 Compute  alpha*Q*pinv(S)'*P'*B  in B using workspace. */

		    i__1 = *n;
		    for (i__ = 1; i__ <= i__1; ++i__) {
			dgemv("NoTranspose", m, rank, &c_b35, &q[q_offset], 
				ldq, &b[i__ * b_dim1 + 1], &c__1, &c_b36, &dwork[1], &c__1);
			dcopy(m, &dwork[1], &c__1, &b[i__ * b_dim1 + 1], &c__1);
/* L70: */
		    }
		} else {
...
/*                 Compute  alpha*Q'*B  in B using workspace. */
		    ...
		    i__1 = *n;
		    for (i__ = 1; i__ <= i__1; ++i__) {
			dgemv("Transpose", m, m, alpha, &q[q_offset], ldq, &
				b[i__ * b_dim1 + 1], &c__1, &c_b36, &dwork[1], &c__1);
			dcopy(m, &dwork[1], &c__1, &b[i__ * b_dim1 + 1], &c__1);
/* L80: */
		    }
...
/*                 Compute  alpha*P*pinv(S)*Q'*B  in B using workspace. */
		    ...
		    i__1 = *n;
		    for (i__ = 1; i__ <= i__1; ++i__) {
			dgemv("Transpose", rank, m, &c_b35, &r__[r_offset], 
				ldr, &b[i__ * b_dim1 + 1], &c__1, &c_b36, &
				dwork[1], &c__1);
			dcopy(m, &dwork[1], &c__1, &b[i__ * b_dim1 + 1], &c__1);
/* L90: */
...
/*                 Compute  alpha*B*Q  in workspace. */
		    ...
		    dgemm("NoTranspose", "NoTranspose", m, n, n, alpha, &b[
			    b_offset], ldb, &q[q_offset], ldq, &c_b36, &dwork[1], m);

/*                 Compute  alpha*B*Q*pinv(S)'. */

		    mb01sd("Column scaling", m, rank, &dwork[1], m, &sv[1], &sv[1]);

/*                 Compute  alpha*B*Q*pinv(S)'*P' in B. */

		    dgemm("NoTranspose", "NoTranspose", m, n, rank, &c_b35, &
			    dwork[1], m, &r__[r_offset], ldr, &c_b36, &b[b_offset], ldb);
		} else {

/*                 Compute  alpha*B*P  in workspace. */

		    dgemm("NoTranspose", "Transpose", m, n, n, alpha, &b[
			    b_offset], ldb, &r__[r_offset], ldr, &c_b36, &dwork[1], m);
...
/*                 Compute  alpha*B*P*pinv(S)*Q' in B. */
		    ...
		    dgemm("NoTranspose", "Transpose", m, n, rank, &c_b35, &
			    dwork[1], m, &q[q_offset], ldq, &c_b36, &b[b_offset], ldb);
		}
	    } else {
		if (tran) {
...
/*                 Compute  alpha*B*Q  in B using workspace. */
		    ...
		    i__1 = *m;
		    for (i__ = 1; i__ <= i__1; ++i__) {
			dgemv("Transpose", n, n, alpha, &q[q_offset], ldq, &
				b[i__ + b_dim1], ldb, &c_b36, &dwork[1], &c__1);
			dcopy(n, &dwork[1], &c__1, &b[i__ + b_dim1], ldb);
/* L100: */
		    }
...
/*                 Compute  alpha*B*Q*pinv(S)'. */
		    ...
		    mb01sd("Column scaling", m, rank, &b[b_offset], ldb, &sv[1], &sv[1]);

/*                 Compute  alpha*B*Q*pinv(S)'*P' in B using workspace. */

		    i__1 = *m;
		    for (i__ = 1; i__ <= i__1; ++i__) {
			dgemv("Transpose", rank, n, &c_b35, &r__[r_offset], 
				ldr, &b[i__ + b_dim1], ldb, &c_b36, &dwork[1],&c__1);
			dcopy(n, &dwork[1], &c__1, &b[i__ + b_dim1], ldb);
/* L110: */
		    }
...
/*                 Compute  alpha*B*P  in B using workspace. */
		    ...
		    i__1 = *m;
		    for (i__ = 1; i__ <= i__1; ++i__) {
			dgemv("NoTranspose", n, n, alpha, &r__[r_offset], 
				ldr, &b[i__ + b_dim1], ldb, &c_b36, &dwork[1],&c__1);
			dcopy(n, &dwork[1], &c__1, &b[i__ + b_dim1], ldb);
/* L120: */
		    }
...
/*                 Compute  alpha*B*P*pinv(S)*Q' in B using workspace. */
		    ...
		    i__1 = *m;
		    for (i__ = 1; i__ <= i__1; ++i__) {
			dgemv("NoTranspose", n, rank, &c_b35, &q[q_offset], 
				ldq, &b[i__ + b_dim1], ldb, &c_b36, &dwork[1],&c__1);
			dcopy(n, &dwork[1], &c__1, &b[i__ + b_dim1], ldb);
/* L130: */
		    }
		}
