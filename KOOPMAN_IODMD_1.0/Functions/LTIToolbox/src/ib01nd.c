/* IB01ND.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int ib01nd(char *meth, char *jobd, integer *nobr, integer *
...
/*     Check the scalar input parameters. */
    ...
    if (! (moesp || n4sid)) {
	*info = -1;
    } else if (moesp && ! (jobdm || lsame(jobd, "N"))) 
	    {
	*info = -2;
    } else if (*nobr <= 0) {
	*info = -3;
...
/* Computing MAX */
		...
		maxwrk = lnobr + lnobr * ilaenv(&c__1, "DGEQRF", " ", &
			lmnobr, &lnobr, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1);
	    } else {

/* Computing MAX */
		i__1 = minwrk, i__2 = lmnobr * 5;
		minwrk = max(i__1,i__2);
/* Computing MAX */
		i__1 = mnobr + mnobr * ilaenv(&c__1, "DGEQRF", " ", &mmnobr, 
			&mnobr, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1), i__2 = 
			mnobr + llnobr * ilaenv(&c__1, "DORMQR", "LT", &
			mmnobr, &llnobr, &mnobr, &c_n1, (ftnlen)6, (ftnlen)2);
		maxwrk = max(i__1,i__2);
/* Computing MAX */
		i__1 = maxwrk, i__2 = mnobr + lnobr * ilaenv(&c__1, "DORMQR",
			 "LN", &mmnobr, &lnobr, &mnobr, &c_n1, (ftnlen)6, (ftnlen)2);
		maxwrk = max(i__1,i__2);
/* Computing MAX */
		i__1 = maxwrk, i__2 = lnobr + lnobr * ilaenv(&c__1, "DGEQRF",
			 " ", &lmmnob, &lnobr, &c_n1, &c_n1, (ftnlen)6, (ftnlen)1);
		maxwrk = max(i__1,i__2);
	    }
...
/*           copy the (1,4) block into (3,3). */
	    ...
	    dlacpy("Upper", &mnobr, &mnobr, &r__[r_offset], ldr, &r__[nr3 + 
		    nr2 * r_dim1], ldr);
	    dlacpy("Full", &mnobr, &lnobr, &r__[nr4 * r_dim1 + 1], ldr, &r__[
		    nr3 + nr3 * r_dim1], ldr);
...
/*           Workspace: need M*NOBR + MAX(M-1,L)*NOBR. */
	    ...
	    mb04od("Upper", &mnobr, &lnobr, &mnobr, &r__[nr2 + nr2 * r_dim1],
		     ldr, &r__[nr3 + nr2 * r_dim1], ldr, &r__[nr2 + nr3 * 
		    r_dim1], ldr, &r__[nr3 + nr3 * r_dim1], ldr, &dwork[itau],
		     &dwork[jwork]);
	    i__1 = mnobr - 1;
	    i__2 = *ldwork - jwork + 1;
	    mb04id(&mmnobr, &mnobr, &i__1, &lnobr, &r__[nr2 * r_dim1 + 1], 
...
/*           final positions, required by SLICOT Library routine  IB01PD. */
	    ...
	    dlacpy("Upper", &mnobr, &mnobr, &r__[nr2 * r_dim1 + 1], ldr, &
		    r__[lmnobr + 1 + r_dim1], ldr);
	    dlacpy("Full", &mnobr, &lnobr, &r__[nr3 * r_dim1 + 1], ldr, &r__[
		    nr2 * r_dim1 + 1], ldr);
	}

/*        Copy [ R_24'  R_34' ]'  in  [ R_22'  R_32' ]'. */

	dlacpy("Full", &lmnobr, &lnobr, &r__[nr2 + nr4 * r_dim1], ldr, &r__[
		nr2 + nr2 * r_dim1], ldr);
...
/*        Workspace: need 2*L*NOBR; prefer L*NOBR + L*NOBR*NB. */
	...
	jwork = itau + lnobr;
	i__1 = *ldwork - jwork + 1;
	dgeqrf(&lmnobr, &lnobr, &r__[nr2 + nr2 * r_dim1], ldr, &dwork[itau], 
...
/*           redefined according to their new position. */
		...
		dswap(&i__, &r__[i__ * r_dim1 + 1], &c__1, &r__[(mnobr + i__)
			 * r_dim1 + 1], &c__1);
		dcopy(&mnobr, &r__[i__ + 1 + (mnobr + i__) * r_dim1], &c__1, 
			&r__[i__ + 1 + i__ * r_dim1], &c__1);
		i__2 = mmnobr - i__;
		dcopy(&i__2, dum, &c__0, &r__[i__ + 1 + (mnobr + i__) * 
			r_dim1], &c__1);
/* L10: */
	    }
...
/*           Workspace: need 2*(M+L)*NOBR. */
	    ...
	    i__1 = mnobr - 1;
	    i__2 = *ldwork - jwork + 1;
	    mb04id(&mmnobr, &mnobr, &i__1, &llmnob, &r__[r_offset], ldr, &
...
/*           Save updated  Y_f  (transposed) in the last block-row of  R. */
	    ...
	    ma02ad("Full", &lmmnob, &lnobr, &r__[nr4 * r_dim1 + 1], ldr, &
		    r__[nr4 + r_dim1], ldr);
...
/*           Workspace: need 4*M*NOBR. */
	    ...
	    dtrcon("1-norm", "Upper", "NonUnit", &mnobr, &r__[r_offset], ldr,
		     &rcond1, &dwork[jwork], &iwork[1], &ierr);
...
/*              U_f is considered full rank and no pivoting is used. */
		...
		dlaset("Full", &mnobr, &llmnob, &c_b50, &c_b50, &r__[nr2 * 
			r_dim1 + 1], ldr);
	    } else {
...
/* L15: */
		    ...
		    i__2 = mnobr - i__;
		    dcopy(&i__2, dum, &c__0, &r__[i__ + 1 + i__ * r_dim1], &
			    c__1);
		    iwork[i__] = 0;
/* L20: */
		}
...
/*              Workspace: need   5*M*NOBR. */

		itau2 = jwork;
		jwork = itau2 + mnobr;
		svlmax = 0.;
		mb03od("QR", &mnobr, &mnobr, &r__[r_offset], ldr, &iwork[1], 
...
/*                         prefer 2*M*NOBR + (M+2*L)*NOBR*NB. */
		...
		i__1 = *ldwork - jwork + 1;
		dormqr("Left", "Transpose", &mnobr, &llmnob, &mnobr, &r__[
...
/*                         prefer 2*M*NOBR + (M+2*L)*NOBR*NB. */
		...
		dlaset("Full", &rank, &llmnob, &c_b50, &c_b50, &r__[nr2 * 
			r_dim1 + 1], ldr);
		i__1 = *ldwork - jwork + 1;
		dormqr("Left", "NoTranspose", &mnobr, &llmnob, &mnobr, &r__[
			r_offset], ldr, &dwork[itau2], &r__[nr2 * r_dim1 + 1],
			 ldr, &dwork[jwork], &i__1, &ierr);
/* Computing MAX */
		i__1 = maxwrk, i__2 = (integer) dwork[jwork] + jwork - 1;
...
/*                      prefer larger. */
	    ...
	    i__1 = mnobr - 1;
	    i__2 = *ldwork - jwork + 1;
	    mb04iy("Left", "NoTranspose", &mmnobr, &llmnob, &mnobr, &i__1, &
		    r__[r_offset], ldr, &dwork[itau], &r__[nr2 * r_dim1 + 1], 
		    ldr, &dwork[jwork], &i__2, &ierr);
/* Computing MAX */
	    i__1 = maxwrk, i__2 = (integer) dwork[jwork] + jwork - 1;
	    maxwrk = max(i__1,i__2);
...
/*                   prefer   M*NOBR + M*NOBR*NB. */
	...
	i__1 = *ldwork - jwork + 1;
	dgeqrf(&mmnobr, &mnobr, &r__[nr2 * r_dim1 + 1], ldr, &dwork[itau], &
		dwork[jwork], &i__1, &ierr);
...
/*                   prefer M*NOBR + 2*L*NOBR*NB. */
	...
	i__1 = *ldwork - jwork + 1;
	dormqr("Left", "Transpose", &mmnobr, &llnobr, &mnobr, &r__[nr2 * 
	...
	i__1 = lnobr - 1;
	i__2 = *ldwork - jwork + 1;
	mb04id(&lmnobr, &lnobr, &i__1, &lnobr, &r__[nr2 + nr3 * r_dim1], ldr,
...
/*        Workspace: need 4*(M+L)*NOBR. */
	...
	dtrcon("1-norm", "Upper", "NonUnit", &lmnobr, &r__[nr2 * r_dim1 + 1],
...
/*              triangle),  R_21  and  R_31  (transposed information). */
		...
		i__1 = mmnobr - 1;
		dlacpy("Lower", &i__1, &mnobr, &r__[nr2 * r_dim1 + 2], ldr, &
			r__[r_dim1 + 2], ldr);
		nrsave = 1;

		i__1 = lmnobr;
		for (i__ = nr2; i__ <= i__1; ++i__) {
		    dcopy(&mnobr, &r__[i__ + 1 + (mnobr + i__) * r_dim1], &
...
/* L40: */
	    ...
	    i__1 = lmnobr - 1;
	    i__2 = lmnobr - 1;
	    dlaset("Lower", &i__1, &i__2, &c_b50, &c_b50, &r__[nr2 * r_dim1 
		    + 2], ldr);

/*           Use QR factorization with column pivoting. */
/*           Workspace: need 5*(M+L)*NOBR. */
...
/* L50: */
	    ...
	    jwork = itau3 + lmnobr;
	    svlmax = 0.;
	    mb03od("QR", &lmnobr, &lmnobr, &r__[nr2 * r_dim1 + 1], ldr, &
...
/*                      prefer 2*(M+L)*NOBR + L*NOBR*NB. */
	    ...
	    i__1 = *ldwork - jwork + 1;
	    dormqr("Left", "Transpose", &lmnobr, &lnobr, &lmnobr, &r__[nr2 * 
...
/*                      prefer 2*(M+L)*NOBR + L*NOBR*NB. */
	    ...
	    i__1 = lmnobr - rank1;
	    dlaset("Full", &i__1, &lnobr, &c_b50, &c_b50, &r__[rank1 + 1 + 
		    nr4 * r_dim1], ldr);
	    i__1 = *ldwork - jwork + 1;
	    dormqr("Left", "NoTranspose", &lmnobr, &lnobr, &lmnobr, &r__[nr2 
		    * r_dim1 + 1], ldr, &dwork[itau3], &r__[nr4 * r_dim1 + 1],
		     ldr, &dwork[jwork], &i__1, &ierr);
/* Computing MAX */
	    i__1 = maxwrk, i__2 = (integer) dwork[jwork] + jwork - 1;
...
/*              Restore the saved transpose matrix from  R_31. */
		...
		i__1 = lmnobr;
		for (i__ = nr2; i__ <= i__1; ++i__) {
		    dcopy(&mnobr, &r__[mnobr + i__ + r_dim1], ldr, &r__[i__ 
...
/*                   prefer larger. */
	...
	i__1 = lnobr - 1;
	i__2 = *ldwork - jwork + 1;
	mb04iy("Left", "NoTranspose", &lmnobr, &lnobr, &lnobr, &i__1, &r__[
		nr2 + nr3 * r_dim1], ldr, &dwork[itau2], &r__[nr2 + nr4 * 
		r_dim1], ldr, &dwork[jwork], &i__2, &ierr);
/* Computing MAX */
	i__1 = maxwrk, i__2 = (integer) dwork[jwork] + jwork - 1;
	maxwrk = max(i__1,i__2);
...
/*                   prefer M*NOBR + L*NOBR*NB. */
	...
	jwork = itau2;
	i__1 = *ldwork - jwork + 1;
	dormqr("Left", "NoTranspose", &mmnobr, &lnobr, &mnobr, &r__[nrsave * 
...
/*                   prefer   L*NOBR + L*NOBR*NB. */
	...
	jwork = itau + lnobr;
	i__1 = *ldwork - jwork + 1;
	dgeqrf(&lmmnob, &lnobr, &r__[nr4 * r_dim1 + 1], ldr, &dwork[itau], &
		dwork[jwork], &i__1, &ierr);

/*        Copy the triangular factor to its final position,  R_22. */

	dlacpy("Upper", &lnobr, &lnobr, &r__[nr4 * r_dim1 + 1], ldr, &r__[
		nr2 + nr2 * r_dim1], ldr);

/*        Restore  Y_f. */

	ma02ad("Full", &lnobr, &lmmnob, &r__[nr4 + r_dim1], ldr, &r__[nr4 * 
		r_dim1 + 1], ldr);
    }
...
/*     Workspace: need 5*L*NOBR. */
    ...
    mb03ud("NoVectors", "Vectors", &lnobr, &r__[nr2 + nr2 * r_dim1], ldr, 
	    dum, &c__1, &sv[1], &dwork[1], ldwork, &ierr);
    if (ierr != 0) {
	*info = 2;
	return 0;
    }
...
/*     columns will then be the singular vectors needed subsequently. */
	...
	i__2 = lmnobr - i__ + 1;
	dswap(&i__2, &r__[i__ + (i__ - 1) * r_dim1], &c__1, &r__[i__ - 1 + 
		i__ * r_dim1], ldr);
/* L70: */
    }
