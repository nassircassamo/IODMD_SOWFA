/* MB02TZ.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int mb02tz(char *norm, integer *n, doublereal *hnorm, 
...
/*     Test the input parameters. */
    ...
    /* Function Body */
    *info = 0;
    onenrm = *(unsigned char *)norm == '1' || lsame(norm, "O");
...
/*     Estimate the norm of inv(H). */
    ...
    *(unsigned char *)normin = 'N';
    ...
    kase = 0;
L10:
    zlacon(n, &zwork[*n + 1], &zwork[1], &hinvnm, &kase);
    if (kase != 0) {
	if (kase == kase1) {
...
/*           Multiply by inv(U). */
	    ...
	    zlatrs("Upper", "No transpose", "Non-unit", normin, n, &h__[
		    h_offset], ldh, &zwork[1], &scale, &dwork[*n + 1], info);
	} else {

/*           Multiply by inv(U'). */

	    zlatrs("Upper", "Conjugate transpose", "Non-unit", normin, n, &
		    h__[h_offset], ldh, &zwork[1], &scale, &dwork[*n + 1], info);
