/* SB03SX.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int sb03sx(char *trana, char *uplo, char *lyapun, integer *
...
/*     Decode and Test input parameters. */
    ...
    nn = *n * *n;
    *info = 0;
    if (! (notrna || lsame(trana, "T") || lsame(trana,
	     "C"))) {
	*info = -1;
    } else if (! (lsame(uplo, "L") || lsame(uplo, 
	    "U"))) {
	*info = -2;
...
/*           Solve op(T)'*Y*op(T) - Y = scale*RHS. */
	    ...
	    sb03mx(trana, n, &t[t_offset], ldt, &dwork[1], n, &scale, &dwork[
		    itmp], &info2);
	} else {
