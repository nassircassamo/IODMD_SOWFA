/* MB04OD.f -- translated by f2c (version 20041007).
...
/* Subroutine */ int mb04od(char *uplo, integer *n, integer *m, integer *p, 
...
/*           its structure. */
	    ...
	    im = min(i__,*p);
	    i__2 = im + 1;
	    dlarfg(&i__2, &r__[i__ + i__ * r_dim1], &a[i__ * a_dim1 + 1], &
		    c__1, &tau[i__]);
...
/*           [ A(1:IM,I+1:N) ]    [ A(1:IM,I+1:N) ]         [ v ] */
	    ...
	    if (*n - i__ > 0) {
		i__2 = *n - i__;
		mb04oy(&im, &i__2, &a[i__ * a_dim1 + 1], &tau[i__], &r__[i__ 
...
/*           [ C(1:IM,:) ]    [ C(1:IM,:) ]         [ v ] */
	    ...
	    if (*m > 0) {
		mb04oy(&im, m, &a[i__ * a_dim1 + 1], &tau[i__], &b[i__ + 
...
/*           [ A(:,I+1:N) ]    [ A(:,I+1:N) ]         [ v ] */
	    ...
	    i__2 = *n - i__;
	    mb04oy(p, &i__2, &a[i__ * a_dim1 + 1], &tau[i__], &r__[i__ + (
...
/* L20: */
	...
	i__1 = *p + 1;
	dlarfg(&i__1, &r__[*n + *n * r_dim1], &a[*n * a_dim1 + 1], &c__1, &
		tau[*n]);
	if (*m > 0) {
...
/*              [   C    ]    [   C    ]         [ v ] */
		...
		mb04oy(p, m, &a[i__ * a_dim1 + 1], &tau[i__], &b[i__ + 
			b_dim1], ldb, &c__[c_offset], ldc, &dwork[1]);
/* L30: */
	    }
