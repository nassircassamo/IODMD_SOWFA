function [B,D,Rnew] = dac2bd_cl(A,C,u,y,r,uhat,yhat,NN,Rold)
...
% 
% Syntax:   
% 	    [B,D,Rnew] = dac2bd_cl(A,C,u,y,r,uhat,yhat)
% 	    [B,D,Rnew] = dac2bd_cl(A,C,u,y,r,uhat,yhat,NN,Rold)
% 					
% Input:
% A,C       Estimated A and C matrices from dmodeiv
% u,y       Noisy input and output data 
% r         External reference signal
...
% Rold      Compressed data matrix from the previous call of cl_bd. 
% 
% Output:
% B,D       Estimated system matrices.
% Rnew      Compressed data matrix for the next call of cl_bd. 
%           Used when analyzing multiple data sequences.
% 
% See also: dac2b_cl, dac2bd_eiv, dac2d_eiv, dordeiv, dmodeiv 
% 
...
mp = max(abs(eig(A)));
if (mp >= 1)
    disp('Note from cl_bd: A matrix is unstable so closed-loop data is assumed.')
    K = place(A',C',(0:n-1)/n);
    K = K';
    A = A - K * C;
